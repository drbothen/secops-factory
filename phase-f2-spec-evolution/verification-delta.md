...
| **VP-HOOK-031** *(NEW v1.14, **P0**; ADV-F2-P11-004 MAJOR; UPDATED v1.14→v1.15 four-guarantee scope, ADV-F2-P12-002 CRITICAL)* | BC-3.03.001 (disposition-guard separate human-comment marker path — P11-004; redesigned P12-002); consumed by BC-5.01.001 (investigate-event) + BC-4.02.001 (update-jira human comment) | none (max prior VP-HOOK = 030; `grep -rn` confirmed absent repo-wide) | **FINALIZED** (the 12-field investigation-markdown path does NOT enter the verdict emitter; **v1.15 (P12-002) four-guarantee redesign:** (a) `autonomy_enabled` absent/≠true → allow-without-marker (no autonomous comment under the kill switch); (b) `disposition != FP` → route to create-review/comment-review (NOT an autonomous comment — TP/BTP surface to human review; closes the autonomous-loop-masquerade bypass); (c) `disposition=FP` + markdown-evaluable floors pass (Indeterminate / forbidden techniques / degraded-silent sensor) → comment marker; (d) `ticket_id` charset-validated on the markdown path (P12-001) — NOT validate_enums/STEP-1a; paired mutants SM-47 (markdown-into-verdict-emitter, P11-004), SM-50 (remove kill-switch gate), SM-51 (remove disposition!=FP route-to-review)) |
...
→ SM-48/49/50/51 + new VP-HOOK-032, all next-free**).** Each
VP-SKILL-050..075 and VP-HOOK-024..032 appears in exactly one *owning* BC.
...
O7 compliance artifact, P12-001/P12-007)** = **35 VPs** for the cycle
...
four-guarantee markdown-path redesign (kill switch / disposition routing / FP comment / ticket_id charset)**;
...
human-comment (12-field investigation-markdown) marker path — the markdown does NOT enter the verdict emitter (P11-004)**), **26 skill properties**
...
P12-002 four-guarantee scope) is FINALIZED P0 (the separate human-comment marker path is convergence-blocking — it
resolves the P11-004 emitter-entry impossibility that would deny a complete 12-field investigation at save time, AND
the P12-002 CRITICAL where the pre-redesign path issued a live comment marker for any disposition under an active kill
switch). **VP-HOOK-032 (NEW v1.15) is FINALIZED P0 — the O7 command_pattern interpolation-safety property is
...
floor-exemption is architectural policy, not a control-flow security branch).** All 45
...
| **SM-51** *(NEW v1.15; occupancy-verified — `grep -rn 'SM-51' .factory/` returned no match before allocation; SM-P12-D → next free after SM-50)* | disposition-guard markdown (Human-Comment) path — disposition!=FP route-to-review rule (D-DEC-008 / P12-002) | **markdown-disposition-route-to-review-removed** (ADV-F2-P12-002 CRITICAL) — remove the `IF parsed_disposition != "FP": GOTO MARKDOWN_CREATE_REVIEW/COMMENT_REVIEW_PATH` routing rule so the markdown path emits a regular `["comment"]` marker for ALL dispositions (the pre-P12-002 behavior). A `Disposition: TP`/`BTP` investigation markdown then gets an AUTONOMOUS comment marker instead of surfacing to human review — the hook cannot evaluate scored_priority/asset_type from a 12-field markdown, so an escalation-worthy finding is silently auto-commented (the masquerade bypass: the loop writes a TP as `investigation-*.md` to reach a weaker gate) | **VP-HOOK-031 disposition-routing vectors (c2)/(c4)** (`Disposition: TP` markdown → create-review/comment-review marker, NOT a regular comment; masquerade: loop writes investigation-TP-*.md with HIGH indicators → review marker, autonomous comment NOT issued) — under the mutant a TP markdown gets a regular comment marker and the mutant dies; vector (c1) (`Disposition: FP` → comment marker) confirms benign FP investigations are unaffected. Separately killable from SM-50 (SM-50 = kill-switch gate absent → comment under kill switch; SM-51 = routing rule absent → comment for TP/BTP even with the kill switch off — distinct branches) |

**New mutation vector count: 45 (SM-9 … SM-51; SM-32 = SM-32a + SM-32b + SM-32-ext as of v1.9; SM-32a re-targeted + SM-32-ext kill vector re-worded at v1.10; +SM-38/SM-39/SM-40 at v1.10; +SM-41/SM-42 at v1.11; +SM-43 at v1.12; +SM-44/SM-45 at v1.13; +SM-46/SM-47 at v1.14; +SM-48/SM-49/SM-50/SM-51 at v1.15).** SM-9..SM-16,
...
markdown-kill-switch-gate-removed sentinel and SM-51 the ADV-F2-P12-002 CRITICAL markdown-disposition-route-to-review-removed
sentinel — both land on the disposition-guard markdown (Human-Comment) path (VP-HOOK-031); a removed kill-switch gate or
routing rule lets the autonomous loop obtain a comment marker under an active kill switch (SM-50) or for a TP/BTP finding
(SM-51) via an artifact-class choice, bypassing a deterministic auth control, so they are on the CRITICAL emitter surface
...
With SM-9..SM-16 + SM-21..SM-22 + SM-28 + SM-29..SM-30 + **SM-31, SM-32a, SM-32b, SM-32-ext, SM-33..SM-51**
KILLED, the marker/authorization path meets the **≥95% require-review** target (SM-31/32a/32b/32-ext/33/34/38/39/41/**44/45/46/47/48/49/50/51**
land squarely on the disposition-guard emitter authorization boundary — **SM-44 on STEP 1a consistency, SM-45 on WRITE_MARKER review-path fail-closed, SM-46 on the scored_priority high-severity floor, SM-47 on the investigation-markdown emitter-entry router, SM-48/SM-49 on the O7 command_pattern charset-validation gate, SM-50/SM-51 on the markdown Human-Comment kill-switch/disposition-routing path**; SM-35/36/37/40/42/43 on the require-review
...
architecture-delta v1.15 body — the schema went 15→17 (P10-001) →**18 fields** (P11-002 `scored_priority`), a
...
authorization/emit-time hardenings, not semantic changes to any invariant). All delta BCs are otherwise
internally consistent with the finalized **35-VP** set (VP-SKILL 001–075, VP-HOOK 024–032; v1.10/v1.11/v1.12 add no
...
and the SM-N catalog (SM-9..SM-51, **45 mutants** with SM-32 = SM-32a + SM-32b + SM-32-ext; +SM-38/SM-39/SM-40 at v1.10; +SM-41/SM-42 at v1.11; +SM-43 at v1.12; +SM-44/SM-45 at v1.13; +SM-46/SM-47 at v1.14; **+SM-48/SM-49/SM-50/SM-51 at v1.15**).
...
*F2 Verification Delta v1.15 complete. **35 VPs** (0 collisions, 0 renumbering): VP-SKILL 001–075, VP-HOOK
