...
introduced: v0.6.0
modified: ["v1.1-ADV-0-501-2026-07-19", "v1.2-ADV-0-601-2026-07-19", "v1.3-ADV-0-702-ADV-0-706-2026-07-19", "v1.4-ADV-0-901-2026-07-19", "v1.5-PRISM-EVIDENCE-2026-07-20", "v1.6-ADV-F2-P3-004-2026-07-20", "v1.7-FV-VP-069-FINALIZED-JOB2-XREF-2026-07-20", "v1.8-version-coherence-sweep-2026-07-21", "v1.9-ADV-F2-P11-004-12field-hard-floor-reconcile-2026-07-22 [ID-sync per FV]", "v1.10-ADV-F2-P15-001-P15-005-markdown-path-no-comment-marker-2026-07-22", "v1.11-ADV-F2-P16-001-gate1-first-human-path-2026-07-22", "v1.12-CV-004-CV-005-version-cite-current-2026-07-23", "v1.13-ADV-F2-P25-001-P25-003-D029-save-always-succeeds-review-marker-model-2026-07-29", "v1.14-ADV-F2-P27-001-P27-002-inv7-structural-retain-infra-fail-loud-2026-07-29", "v1.15-P32-coherence-sweep-2026-09-03"]
deprecated: null
deprecated_by: null
replacement: null
retired: null
...
> **Revision history:**
> - v1.15 (2026-09-03): P32-coherence-sweep — live-body version pin updates only. BC-3.03.001 cross-references updated from v1.36→v1.42 in Invariant #7 body (both occurrences). BC-3.01.001 cross-reference updated from v1.22→v1.25 in Invariant #7 confidence footer. BC-3.03.001 v1.34 citation in Invariant #7 NOTE (verdict-class path) left intentionally unchanged. No behavior or semantics changed. P32-coherence-sweep.
> - v1.14 (2026-07-29): Pass-27 adversarial remediation burst 24 — P27-001/P27-002 propagation. **(1) Invariant #7 re-scoped (P27-001/P27-002):** Dropped the absolute "no deny is possible on the markdown path regardless of disposition or sensor state" phrasing. Re-scoped to: no **disposition-/hard-floor-based** deny is possible on the markdown path for a **STRUCTURALLY COMPLETE** investigation (all 12 ICD-203 headings + Alternatives-Considered + charset validation passes); structural ICD-203 completeness, Alternatives-Considered, and charset-injection guards are retained and can still deny (EC-004/EC-010 — pre-D-029-routing structural gates); AND infrastructure marker-write failure on the review path (MARKER-WRITE-FAILED) fails loud rather than silently dropping the escalation — infrastructure-failure denies are DISTINCT from D-029's content/disposition guarantee (P27-001/P27-002). **(2) BC-3.03.001 version reference updated:** v1.34 → v1.36 in Invariant #7 body and Confidence footer. P27-001, P27-002.
> - v1.13 (2026-07-29): Pass-25 adversarial remediation burst 22 — P25-001/D-029 propagation + P25-003 cite sweep. **(1) Invariant #7 rewritten (D-029 save-always-succeeds):** GATE-1-FIRST HUMAN-PATH model retired. "MUST NOT be denied" now holds structurally under D-029 (no deny possible on the markdown path), not through Gate 1 short-circuiting. D-029 routing: (a) FP + `hard_floor_triggered=false` → allow-without-marker (unchanged); (b) non-FP/PARSE_FAIL (any `autonomy_enabled` value) → MARKDOWN_REVIEW_PATH → review marker issued (create-review or comment-review; `jr issue create/comment` is review-marker-gated); (c) FP + `hard_floor_triggered=true` (hard floor wins) → MARKDOWN_REVIEW_PATH; (d) hard-floor conditions (Indeterminate, T1003/T1068/T1021/T1041, degraded/silent) → `hard_floor_triggered=true` + routing annotation; Write ALWAYS succeeds. Previous (v1.11/P16-001 — GATE-1-FIRST model) block added. **(2) P25-003 cite sweep:** "BC-3.03.001 v1.24 Invariant #4" → "BC-3.03.001 v1.34"; "verification-delta.md v1.18" → "verification-delta (VP-HOOK-031, D-029 scope)". P25-001, P25-003, D-029.
> - v1.12 (2026-07-23): CV-004/CV-005 consistency-validator remediation — version cross-reference refresh. Updated live-body cross-references in Invariant #7 and DI-013-RESOLVED footer: BC-3.03.001 v1.23→v1.24 (three occurrences — Invariant #7 body and confidence footnote and DI-013 footer); BC-3.01.001 v1.17→v1.22 (Invariant #7 confidence footnote). Historical, revision-history, and Previous-block cites left intact.
...
7. **[UPDATED v1.5; reconciled v1.9 P11-004; rewritten v1.10 P13-001; gate-1-first v1.11 P16-001; D-029 save-always-succeeds v1.13; P27-001/P27-002 structural-retain-infra-fail-loud v1.14]** Stage 7 includes a Jira action (comment, create, or review marker) to surface the investigation finding. **DI-013 RESOLVED (D-DEC-001):** The Jira action routing via the **Separate Human-Comment Marker Path** (BC-3.03.001 v1.42) follows the **D-029 SAVE-ALWAYS-SUCCEEDS** model. The investigation Write **always succeeds** for **ALL dispositions** (FP, TP, BTP, Indeterminate, or any other). **MUST NOT be denied:** now holds **structurally** under D-029 — no **disposition-/hard-floor-based** deny is possible on the markdown path for a **STRUCTURALLY COMPLETE** investigation (all 12 ICD-203 headings + Alternatives-Considered + charset validation passes); **structural ICD-203 completeness, Alternatives-Considered, and charset-injection guards are retained and can still deny (EC-004/EC-010 — pre-D-029-routing structural gates)**; AND **infrastructure marker-write failure on the review path (MARKER-WRITE-FAILED) fails loud** rather than silently dropping the escalation — infrastructure-failure denies are DISTINCT from D-029's content/disposition guarantee (P27-001/P27-002). **D-029 routing** (see BC-3.03.001 v1.42 Separate Human-Comment Marker Path for authoritative pseudocode): (a) FP + no hard floor (`hard_floor_triggered=false`) → **allow-without-marker** (Write succeeds; no Jira action authorized; subsequent `jr issue comment` falls to human permission-approval as before); (b) non-FP/PARSE_FAIL (any `autonomy_enabled` value, any `hard_floor_triggered` value) → **MARKDOWN_REVIEW_PATH** review marker (create-review or comment-review issued; `jr issue create --label REVIEW-REQUIRED` or `jr issue comment` is review-marker-gated via require-review — D-DEC-012 Option A EXEMPT regardless of `autonomy_enabled`); (c) FP + hard floor (`hard_floor_triggered=true`, e.g., Indeterminate disposition in content, forbidden technique T1003/T1068/T1021/T1041, degraded/silent sensor) → **MARKDOWN_REVIEW_PATH** (hard floor wins over FP disposition; review marker issued; D-029). `autonomy_enabled` is not a routing gate under D-029. **NOTE (P11-004):** Verdict-only fields (`scored_priority ∈ {HIGH,CRIT}` and critical/unknown `asset_type`) are NOT present in a 12-field investigation markdown and are NOT checked on this path — those floors apply only to the verdict-class path (BC-3.03.001 v1.34 PC#1). **Verification property consumed: VP-HOOK-031 (FINALIZED P0 per verification-delta (VP-HOOK-031, D-029/P27-001/P27-002 scope)) — this invariant is a consumer of the Separate Human-Comment Marker Path property; D-029 save-always-succeeds guarantees the investigation Write succeeds for all dispositions (structurally complete investigations only — P27-001); structural completeness and charset guards are pre-routing structural DENY gates (EC-004/EC-010) not covered by D-029; infrastructure marker-write failure on the review path (MARKER-WRITE-FAILED) is fail-closed (P27-002); non-FP dispositions route to MARKDOWN_REVIEW_PATH (review marker issued); FP + no hard floor → allow-without-marker; no disposition from a human analyst save yields an autonomous comment marker (P13-001 MARKDOWN_COMMENT_PATH ELIMINATED). Paired mutant SM-47 (markdown-routed-into-verdict-emitter) is the kill target for compliant-save-allowed and no-validate_enums-on-markdown vectors.**

   > **Previous (v1.11/P16-001 — GATE-1-FIRST model):** "investigate-event writes investigation-*.md ONLY as a human analyst who never injects `autonomy_enabled: true`; therefore GATE 1 fires for every real invocation — `autonomy_enabled` absent → allow-without-marker for ALL dispositions; no Jira action marker of any kind is issued; `jr issue comment` falls to human permission-approval." Under D-029: Gate 1 short-circuiting is eliminated; routing is by disposition and `hard_floor_triggered` regardless of `autonomy_enabled`; non-FP saves now route to MARKDOWN_REVIEW_PATH and may issue a review marker.
   ...
   Confidence: D-DEC-001 binding decision (architecture-delta.md v1.1); BC-3.01.001 v1.25 PC#2 (marker-validation allow path); BC-3.03.001 v1.42 (Separate Human-Comment Marker Path — D-029 save-always-succeeds, P27-001/P27-002 structural-retain-infra-fail-loud, P22-001 / P13-001).
