...
| SM-2 | require-review **write-block if-block** (delete `jr issue assign` / `jr issue create`) | delete `jr issue assign` / `jr issue create` from the write-block | **LARGELY MITIGATED (PR #13/#15).** With the fail-closed default in place and the write-block now evaluated first (PR #15), deleting these lines no longer changes behavior (unknown subcommand still denies) — the mutant is now near-equivalent. The SEC-002 test (`jr issue duplicate` → deny) guards the fail-closed default, and **PR #17 (DI-005) added dedicated positive-deny tests for assign and create**. Fully guarded now. | **RESOLVED** (was HIGH) |
