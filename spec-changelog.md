...
### F2 Pass-28 Remediation Edits — Burst 25 (2026-07-29) — spec remains 1.1.0

Burst 25 (2026-07-29, pass-28 remediation): P28-001 WRITE_MARKER per-path variable definedness — markdown review path now assigns markdown_parsed_disposition=parsed_disposition + explicit link_target=null; dead vars removed (markdown_org_slug_field/markdown_asset_type/asset_type_field); two false setup-block comments fixed; per-path variable-definedness table added; SM-80 (marker-verdict-field-leak) kill now GENUINE (was vacuous pre-fix — markdown_parsed_disposition was never assigned). P28-002 org_slug added to operational-metadata roster + validate_enums() presence check (absent/empty → deny) + ASM-008-class residual (DI-017); SM-81 (org-slug-presence-check-removed) allocated. Obs: multi-org get_org_slug_from_config audit-attribution residual noted in P26-003 block; [process-gap] WRITE_MARKER-path variable-definedness grep gate added to FV methodology. Versions: BC-3.03.001 v1.37; arch-delta v1.30, verif-delta v1.30, prd-delta v1.28. SM roster line → "SM-9..SM-81 (74 allocated / 73 live; SM-32=32a+32b+32-ext; SM-55 reserved-skipped; SM-50 retired)".

| File | Old Version | New Version | Root Finding |
|------|-------------|-------------|--------------|
| phase-0-ingestion/behavioral-contracts/BC-3.03.001.md | v1.36 | v1.37 | P28-001: WRITE_MARKER markdown path variable-definedness fix — assigns markdown_parsed_disposition=parsed_disposition + explicit link_target=null in both markdown setup blocks; dead vars removed (markdown_org_slug_field/markdown_asset_type/asset_type_field); per-path variable-definedness table added; SM-80 kill now GENUINE (was vacuous: markdown_parsed_disposition was never assigned on markdown path pre-fix). P28-002: org_slug added to operational-metadata validation roster; validate_enums() presence check (absent/empty → deny); SM-81 (org-slug-presence-check-removed) allocated. |
| phase-f2-spec-evolution/architecture-delta.md | v1.29 | v1.30 | P28-001: WRITE_MARKER pseudocode per-path variable-definedness corrections (markdown path assigns markdown_parsed_disposition=parsed_disposition + link_target=null); dead var cleanup (markdown_org_slug_field/markdown_asset_type/asset_type_field); two false setup-block comments fixed; per-path definedness table. P28-002: org_slug validate_enums() presence check; ASM-008-class residual (DI-017) documented. Multi-org get_org_slug_from_config audit-attribution residual noted in P26-003 block. |
| phase-f2-spec-evolution/verification-delta.md | v1.29 | v1.30 | P28-001: SM-80 kill reclassified GENUINE (was vacuous pre-fix: markdown_parsed_disposition was never assigned on markdown path); per-path reachability analysis. P28-002: SM-81 allocated (org-slug-presence-check-removed). SM recap 73→74 alloc (72→73 live). [process-gap] WRITE_MARKER-path variable-definedness grep gate added to FV methodology §6. |
| phase-f2-spec-evolution/prd-delta.md | v1.27 | v1.28 | P28-002: org_slug operational-metadata roster entry; version-coherence: BC-3.03.001 v1.36→v1.37; §5 Live-BC version anchors updated. |

---

### F2 Pass-27 Remediation Edits — Burst 24 (2026-07-29) — spec remains 1.1.0

Burst 24 (2026-07-29, pass-27 remediation): P27-001 structural-deny vs disposition-value-parse distinction — structural ICD-203 completeness failure (missing heading / missing Alternatives-Considered / charset) → structural DENY (EC-004/EC-010), fires BEFORE D-029 routing (D-029 item-1 '<12 headings → route to review' arch error corrected); all-12-headings-present + non-canonical Disposition value → allow + MARKDOWN_REVIEW_PATH; BC-5.01.001 Inv#7 + BC-4.02.001 PC#4 re-scoped (drop absolute 'always succeeds for ALL dispositions', qualify to no-disposition/hard-floor-deny + structural-guards-retained + infra-marker-write-fail-loud). P27-002 path-aware WRITE_MARKER (markdown path: org_slug from config, disposition.verdict from markdown_parsed_disposition, severity/asset_type null, is_review_path=TRUE → MARKER-WRITE-FAILED deny on infra failure; link_target self-computing; false L565 claim deleted). P27-003 parse_autonomy_enabled annotation (defense-in-depth only). [ID per FV]→SM-77 resolved. SM-79 (structural-deny-downgraded-to-review) + SM-80 (markdown-marker-verdict-field-leak) allocated. Versions: BC-3.03.001 v1.36, BC-5.01.001 v1.14, BC-4.02.001 v1.20; arch-delta v1.29, verif-delta v1.29, prd-delta v1.27. SM roster line → "SM-9..SM-80 (73 allocated / 72 live; SM-32=32a+32b+32-ext; SM-55 reserved-skipped; SM-50 retired)".

| File | Old Version | New Version | Root Finding |
|------|-------------|-------------|--------------|
| phase-0-ingestion/behavioral-contracts/BC-3.03.001.md | v1.35 | v1.36 | P27-001: structural-deny vs disposition-value-parse distinction finalized — structural ICD-203 incompleteness (missing heading / missing Alternatives-Considered / charset) → DENY (EC-004/EC-010) fires BEFORE D-029 routing (D-029 item-1 '<12 headings→route to review' arch error corrected; consistent with enrichment-completeness co-fire); all-12-headings-present + non-canonical Disposition value → allow + MARKDOWN_REVIEW_PATH. SM-79 (structural-deny-downgraded-to-review mutant) allocated. |
| phase-0-ingestion/behavioral-contracts/BC-5.01.001.md | v1.13 | v1.14 | P27-001: Inv#7 re-scoped — drop absolute 'always succeeds for ALL dispositions'; qualify to no-disposition/hard-floor-deny + structural-guards-retained + infra-marker-write-fail-loud. |
| phase-0-ingestion/behavioral-contracts/BC-4.02.001.md | v1.19 | v1.20 | P27-001: PC#4 re-scoped — drop absolute 'always succeeds'; qualify to no-disposition/hard-floor-deny + structural-guards-retained + infra-marker-write-fail-loud. |
| phase-f2-spec-evolution/architecture-delta.md | v1.28 | v1.29 | P27-002: path-aware WRITE_MARKER on markdown path — org_slug from config (not verdict), disposition.verdict from markdown_parsed_disposition, severity/asset_type null, is_review_path=TRUE; MARKER-WRITE-FAILED deny on infra failure (infrastructure deny distinct from content deny); link_target self-computing; false L565 claim deleted. SM-80 (markdown-marker-verdict-field-leak mutant) allocated. |
| phase-f2-spec-evolution/verification-delta.md | v1.28 | v1.29 | P27-002: path-aware WRITE_MARKER vectors; SM-79/SM-80 allocated; SM-77 ID-mapping resolved ([ID per FV]→SM-77); SM recap 71→73 alloc (70→72 live). |
| phase-f2-spec-evolution/prd-delta.md | v1.26 | v1.27 | P27-003: parse_autonomy_enabled annotation (defense-in-depth only); version-coherence: BC-3.03.001 v1.35→v1.36, BC-5.01.001 v1.13→v1.14, BC-4.02.001 v1.19→v1.20; §5 Live-BC version anchors updated. |
