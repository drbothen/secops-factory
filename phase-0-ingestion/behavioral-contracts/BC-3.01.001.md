   ...
   **Marker-validation algorithm (D-DEC-001 v2.0 — [UPDATED v1.14] iterative-consume):**
   ...
   for each file F in marker_dir/*.marker.json:
     ...
     (6) authorized_operations scope check — EXACT-TYPE MATCHING (ADV-F2-P6-001):
         ...
         #       return True
         #   return False
         # A --summary value containing "--label REVIEW-REQUIRED" as free text is NOT a standalone --label
         # flag token; the token boundary check prevents a bare substring match inside an argument value.
         # (EC-024: false-deny prevention for regular creates with label text in --summary.)
         #
...
| EC-023 | **Consumer anti-fungibility cross-check (ADV-F2-P6-001)** — two directions: (A) `jr issue create --project PRISM-DEMO --label REVIEW-REQUIRED --summary "..."` with a regular `["create"]` marker in the store (marker `authorized_operations: ["create"]`, no review label in the command_pattern). (B) `jr issue create --project PRISM-DEMO --summary "FP finding"` (no `--label` flag) with a `["create-review"]` marker in the store. | (A) Write-block matched; marker scan finds `["create"]` candidate; step (5) anchored match: the regular create pattern `^jr ... issue create --project PRISM-DEMO( \|$)` does NOT match a command containing `--label REVIEW-REQUIRED` because `--label` appears before the `( \|$)` anchor, causing the regular pattern to fail at step 5 (structural defense-in-depth). If for any reason step 5 passes: step (6a) fires — `structural_label_check(command)=true` (standalone `--label REVIEW-REQUIRED` token pair found), `authorized_operations==["create"]` → CONTINUE (anti-fungibility). No valid marker consumed; deny. (B) Write-block matched; marker scan finds `["create-review"]` candidate; step (5) anchored match: the create-review pattern `^jr ... issue create --project PRISM-DEMO --label (REVIEW-REQUIRED\|BLIND-SPOT)( \|$)` does NOT match `jr issue create --project PRISM-DEMO` (no `--label` clause). Step 5 fails (structural defense-in-depth). If step 5 passes: step (6a) fires — `structural_label_check(command)=false`, `authorized_operations==["create-review"]` → CONTINUE. No valid marker consumed; deny. Both directions: create-review marker cannot authorize a non-review-labeled jr create; regular create marker cannot authorize a review-labeled jr create. |
| EC-024 | **False-deny prevention (ADV-F2-P7-005)** — `jr issue create --project PRISM-DEMO --summary "rule matched literal --label REVIEW-REQUIRED in alert payload"` with a valid `["create"]` regular marker (`command_pattern: "^jr (--output json )?issue create --project PRISM-DEMO( \|$)"`, `ticket_id: null`). The string `--label REVIEW-REQUIRED` appears only inside the `--summary` argument value, NOT as a standalone `--label` flag token. | Write-block matched; step (5) anchored match: regular create pattern `^jr ... issue create --project PRISM-DEMO( \|$)` PASSES (no `--label` flag token at the flag position, only text inside `--summary`). Step (6): `structural_label_check(command)` — tokenize on whitespace: `["jr", "issue", "create", "--project", "PRISM-DEMO", "--summary", "rule", "matched", "literal", "--label", "REVIEW-REQUIRED", "in", "alert", "payload"]`. The `--label` token at position 9 is followed by `REVIEW-REQUIRED` at position 10 — however, after shell expansion the `--summary "..."` argument is a single token to the hook (the shell strips quotes before hook sees the command). If the shell delivered it as separate tokens, structural_label_check fires; in practice `--summary` quoting collapses its value. **In any case:** the old substring check would have returned `true` (finding `--label REVIEW-REQUIRED` anywhere in the raw command string) → false-deny under the old v1.18 behavior. The structural token check correctly scopes to standalone flag tokens; a `--label REVIEW-REQUIRED` sequence embedded inside `--summary` text is NOT a standalone flag pair. `has_review_label=false`; `authorized_operations==["create"]`; step (6a) does NOT block; marker consumed; `permissionDecision: allow`. |
...
| `jr issue create --project PRISM-DEMO --summary "alert payload contains --label REVIEW-REQUIRED text"` (no standalone `--label` flag) with valid `["create"]` regular marker (`command_pattern: "^jr (--output json )?issue create --project PRISM-DEMO( \|$)"`) | `permissionDecision: allow`; marker consumed (renamed .used); audit log appended; `structural_label_check` returns false (no standalone `--label REVIEW-REQUIRED` token pair at flag position); old substring check would have returned true → false-deny (ADV-F2-P7-005 false-deny prevention — EC-024) | happy-path (EC-024, false-deny prevention) |
