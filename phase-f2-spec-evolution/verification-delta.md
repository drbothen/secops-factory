...
| **VP-HOOK-031** *(NEW v1.14, **P0**; ADV-F2-P11-004 MAJOR)* | BC-3.03.001 (disposition-guard separate human-comment marker path — P11-004); consumed by BC-5.01.001 (investigate-event) + BC-4.02.001 (update-jira human comment) | none (max prior VP-HOOK = 030; `grep -rn` confirmed absent repo-wide) | **FINALIZED** (the 12-field investigation-markdown path does NOT enter the verdict emitter; emits a comment-scoped marker gated ONLY on 12-field completeness + markdown-evaluable hard floors — Indeterminate disposition / forbidden techniques / degraded-silent sensor — NOT validate_enums/STEP-1a; analyst CAN save a compliant 12-field investigation; paired mutant SM-47) |
...
new VP-HOOK-030 / VP-SKILL-075; v1.14's SM-P11-A + unallocated VP → SM-46/47 + new VP-HOOK-031, all next-free).** Each
VP-SKILL-050..075 and VP-HOOK-024..031 appears in exactly one *owning* BC.
...
| **VP-HOOK-031** *(NEW v1.14, **P0**; ADV-F2-P11-004 MAJOR)* | **Separate human-comment marker path (investigation-markdown; P11-004):** the 12-field ICD-203 investigation-markdown path (`*investigation-*.md`) does **NOT** enter the verdict emitter — it executes a SEPARATE, minimal comment-scoped marker path gated ONLY on (a) 12-field completeness (heading-anchored grep) + (b) the markdown-evaluable hard floors — `Disposition: Indeterminate`, any forbidden technique ∈{T1003,T1068,T1021,T1041} in Attack Techniques, `Sensor Health Status: degraded\|silent` — and does **NOT** call `validate_enums()` / STEP 1a (both reference verdict-only fields absent from a 12-field markdown, incl. `severity`/`asset_type`/`ticket_action_type`/`native_severity`/`sensor_family`/`scored_priority`). If all markdown-evaluable floors pass → parse `ticket_id` from the markdown and emit a comment-scoped marker (`authorized_operations:["comment"]`); the analyst's investigation Write is **NOT denied**. If a markdown-evaluable floor fires → `MARKDOWN-HARD-FLOOR` audit entry + deny with an actionable reason (route to human review). This resolves the P11-004 impossibility where the pass-10 emitter's `validate_enums()` would deny a complete 12-field investigation at save time because verdict-only fields are absent. | B-BEH | disposition-guard.sh stdin `{tool_input.file_path=artifacts/investigations/investigation-ALERT-001.md, content}`. **Vectors:** (a) **compliant save** — complete 12-field markdown, `Disposition: TP` (non-Indeterminate), no forbidden technique, healthy/degraded sensor → assert a `["comment"]` marker bound to the parsed `ticket_id` is emitted AND the Write is **NOT denied** (analyst not blocked); (b) **markdown-hard-floor denies** — (b1) `Disposition: Indeterminate` → `MARKDOWN-HARD-FLOOR` deny; (b2) `T1003` in Attack Techniques → `MARKDOWN-HARD-FLOOR` deny; (b3) `Sensor Health Status: degraded` (and `silent`) → `MARKDOWN-HARD-FLOOR` deny; (c) **NO validate_enums/STEP-1a on markdown** — a complete 12-field markdown with NO `scored_priority`/`severity`/`sensor_family` (verdict-only fields) is **NOT** denied by `validate_enums()`/STEP 1a (no false enum/SEVERITY-MISMATCH deny for absent verdict-only fields); missing one of the 12 ICD-203 fields → 12-field completeness deny (NOT an enum-validation deny). Paired mutant **SM-47** (route the 12-field investigation-markdown into the verdict emitter — i.e. run `validate_enums()`/STEP 1a on it) killed by vectors (c)/(a): under the mutant a compliant 12-field markdown hits `validate_enums()`, which denies on the absent `severity`/`scored_priority`/etc. verdict-only fields → the analyst's save is wrongly DENIED → mutant dies. | BC-3.03.001 (disposition-guard separate human-comment marker path, P11-004); consumed by BC-5.01.001 (investigate-event) + BC-4.02.001 (update-jira human comment) |
...
human-comment marker path, P11-004)** = **34 VPs** for the cycle
...
HIGH/CRIT floor +SM-46) in place**).
...
Strategy mix: **8 hook properties** (VP-HOOK-024/025/026/027/028/029/**030**/**031** — CRITICAL/HIGH enforcement;
...
verdict.native_severity", NOT ground-truth enforcement); VP-HOOK-031 (NEW v1.14) is FINALIZED P0 (the separate
human-comment marker path is convergence-blocking — it resolves the P11-004 emitter-entry impossibility that would
deny a complete 12-field investigation at save time).**
...
SM-32a + SM-32b + SM-32-ext). No re-target/renumber this pass.** All 41
...
**New mutation vector count: 41 (SM-9 … SM-47; SM-32 = SM-32a + SM-32b + SM-32-ext as of v1.9; SM-32a re-targeted + SM-32-ext kill vector re-worded at v1.10; +SM-38/SM-39/SM-40 at v1.10; +SM-41/SM-42 at v1.11; +SM-43 at v1.12; +SM-44/SM-45 at v1.13; +SM-46/SM-47 at v1.14).** SM-9..SM-16,
...
at the ≥95% bar.**
With SM-9..SM-16 + SM-21..SM-22 + SM-28 + SM-29..SM-30 + **SM-31, SM-32a, SM-32b, SM-32-ext, SM-33..SM-47**
KILLED, the marker/authorization path meets the **≥95% require-review** target (SM-31/32a/32b/32-ext/33/34/38/39/41/**44/45/46/47**
land squarely on the disposition-guard emitter authorization boundary — **SM-44 on STEP 1a consistency, SM-45 on WRITE_MARKER review-path fail-closed, SM-46 on the scored_priority high-severity floor, SM-47 on the investigation-markdown emitter-entry router**; SM-35/36/37/40/42/43 on the require-review
...
architecture-delta v1.14 body — the schema went 15→17 (P10-001) →**18 fields** (P11-002 `scored_priority`), a
...
internally consistent with the finalized **34-VP** set (VP-SKILL 001–075, VP-HOOK 024–031; v1.10/v1.11/v1.12 add no
...
to a consistency VP, and extends VP-HOOK-025 / VP-HOOK-026**)
and the SM-N catalog (SM-9..SM-47, **41 mutants** with SM-32 = SM-32a + SM-32b + SM-32-ext; +SM-38/SM-39/SM-40 at v1.10; +SM-41/SM-42 at v1.11; +SM-43 at v1.12; **+SM-44/SM-45 at v1.13; +SM-46/SM-47 at v1.14**).
...
*F2 Verification Delta v1.14 complete. **34 VPs** (0 collisions, 0 renumbering): VP-SKILL 001–075, VP-HOOK
