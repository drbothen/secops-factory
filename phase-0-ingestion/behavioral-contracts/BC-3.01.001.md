...
| EC-026 | **Link anti-fungibility (ADV-F2-P18-001 / D-020)** — Two directions: (A) `jr issue link SEC-123 SEC-456` with a `["comment"]` marker in the store (`authorized_operations: ["comment"]`, `command_pattern: "^jr (--output json )?issue comment SEC-123 "`). (B) `jr issue comment SEC-123 "msg"` with a `["link"]` marker in the store (`authorized_operations: ["link"]`, `command_pattern: "^jr (--output json )?issue link SEC-123 SEC-456( \|$)"`). | (A) **DENY:** Write-block matched (`jr issue link `); marker scan finds `["comment"]` candidate; step (5) anchored match: link pattern `^jr (--output json )?issue link SEC-123 SEC-456( \|$)` does NOT match `jr issue link SEC-123 SEC-456` — wait, command does match; step (6) exact-type check: command is a `jr issue link` command → accept ONLY `["link"]`; `authorized_operations==["comment"]` → CONTINUE (type mismatch); no valid marker consumed; `permissionDecision: deny`. (B) **DENY:** Write-block matched (`jr issue comment `); marker scan finds `["link"]` candidate; step (5) anchored match: `^jr (--output json )?issue link SEC-123 SEC-456( \|$)` does NOT match `jr issue comment SEC-123 "msg"` — step 5 fails; no valid marker consumed; `permissionDecision: deny`. A `["link"]` marker cannot authorize a comment command and a `["comment"]` marker cannot authorize a link command (D-020 anti-fungibility). |
