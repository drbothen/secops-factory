...
> the architecture-delta **v1.12** §8.21 FORMAL-VERIFIER LIST. **One new SM id (occupancy re-verified, Lesson 8
...
> is **6 VP-HOOK (024–029) + 25 VP-SKILL** (grand total 31 correct); corrected to **6 hook / 25 skill**. And the
...
This is operationalized for the severity dimension of `hard_floor_applies()` by **VP-HOOK-030** (disposition-guard
STEP 1a `SEVERITY-MISMATCH`, the O6 compliance artifact) — the hook keys the hard floor on the hook-recomputed
severity, never on raw `verdict.severity`. **Any future hook invariant whose inputs are LLM-derived MUST apply
O6:** either carry the raw source + recompute deterministically, or cross-validate against a hook-observable
ground truth, with a paired mutant proving an input-trust regression is detectable. (`verdict.asset_type` is the
one remaining LLM-trust surface in the hard floor: enum-membership is enforced NOW, but the prism-sourced
`prism_asset_class` cross-validation is **ASM-008-DEFERRED** — a documented residual, not an O6 exemption.)
...
| VP-HOOK-025 | BC-10.01.001 (defines) + BC-3.03.001 (enforces) | none — shared reference, not a duplicate assignment | FINALIZED (**17 fields** — 12 ICD-203 + severity[13]/asset_type[14]/ticket_action_type[15]/**native_severity[16]/sensor_family[17]** (P10-001); **v1.5: validate_enums() membership legs; v1.13: fields 16/17 presence + sensor_family enum**) |
...
| **VP-HOOK-030** *(NEW v1.13, **P0**)* | BC-3.03.001 (disposition-guard STEP 1a — D-DEC-008/D-DEC-013 SEVERITY-MISMATCH) + BC-10.01.001 (17-field schema, fields 16/17 — P10-001) | none (max prior VP-HOOK = 029; `grep -rn` confirmed absent repo-wide) | **FINALIZED** (STEP 1a hook-side severity re-normalization; O6 compliance artifact — hard_floor_applies() keys on hook-recomputed severity, SEVERITY-MISMATCH deny on recomputed≠verdict.severity; paired mutant SM-44) |
...
new VP-HOOK-030 / VP-SKILL-075, all next-free).** Each
VP-SKILL-050..075 and VP-HOOK-024..030 appears in exactly one *owning* BC.
...
| VP-SKILL-051 | Prism version gate: `prism --version` parsed and compared to `1.0.0-rc.1`; below → halt with version-gate error, no MCP write; at/above → proceed to dual MCP write | B-INT | prism-version-check.sh with mocked `prism --version`; assert halt vs proceed + no settings write on halt | BC-6.01.001 (v1.6) |
...
SEVERITY-MISMATCH, VP-SKILL-075 operator-boundary cron-exit)** = **33 VPs** for the cycle
...
VP-SKILL-059 structural→behavioral, and RENAMES/extends VP-SKILL-064's carve-out @test names**).
...
Strategy mix: **7 hook properties** (VP-HOOK-024/025/026/027/028/029/**030** — CRITICAL/HIGH enforcement;
...
MARKER-WRITE-FAILED review-path fail-closed vectors; VP-HOOK-030 is the NEW v1.13 P0 STEP 1a SEVERITY-MISMATCH
re-normalization property — the O6 compliance artifact, sibling to VP-HOOK-025/026 on the disposition-guard
enforcement surface**), **26 skill properties**
(VP-SKILL-050/051, 052–063 [12], 064–075 [12]; **VP-SKILL-075 is the NEW v1.13 operator-boundary
...
ACCEPTED lifecycle — an F1-inherited VP whose strategy was strengthened, not a new adjudication).**
...
presence in an investigation file must **NOT** trigger a wrong-class 17-field deny (SM-30 is the paired
over-strict mutant). (ii) if the file is a verdict file (verdict path/extension OR `tool_input.content`
parses as JSON via `jq empty`) → JSON key-presence + type check for **ALL 17 fields**. Key-presence
uses `jq -e 'has("<field>")'` (NOT `!= null`, so a present-null key is distinguishable from an absent
one — fail-closed deny if any of the 17 `has()` returns false).
...
re-run `NORMALIZE_SEVERITY(native_severity, sensor_family)` and cross-check `verdict.severity` (the O6
SEVERITY-MISMATCH deny, owned by VP-HOOK-030; see the STEP 1a note below).** Per-field type assertions (the original 12):
...
`severity` string∈{LOW,MEDIUM,HIGH,CRITICAL} (the LLM-reported severity — **v1.13/P10-001: NO LONGER the
input to the hard-floor check; STEP 1a cross-checks it against `NORMALIZE_SEVERITY(native_severity,
sensor_family)` and `hard_floor_applies()` keys on the hook-RECOMPUTED severity** — NOT `confidence`,
...
→ fail-closed deny — an enum deny, NOT a SEVERITY-MISMATCH). Malformed JSON →
fail-closed deny (mirrors BC-3.01.001 EC-020 marker handling). This is implementable in bash
with `jq` only — no new dependency. Enforced per BC-3.03.001 (post-pass-10) PC#1/2/3 (dual-path) and
BC-10.01.001 v1.14 Invariant #9 (**17-field list** + `autonomy_enabled` operational field).

**[NEW v1.13 — STEP 1a hook-side severity re-normalization, SEVERITY-MISMATCH (ADV-F2-P10-001 CRITICAL /
O6 standing rule §0; owned by VP-HOOK-030).]** After `validate_enums()` (STEP 1) confirms fields 16/17 are
present and well-typed, disposition-guard STEP 1a re-runs `recomputed_severity =
NORMALIZE_SEVERITY(verdict.native_severity, verdict.sensor_family)` using the SAME D-DEC-013 deterministic
table the monitoring loop used at Stage-1 INGEST (a pure lookup; no prism data dependency). **IF
`recomputed_severity != verdict.severity` → WRITE a `SEVERITY-MISMATCH` audit entry naming both values +
emit `deny` (the verdict Write is refused BEFORE any marker decision).** `hard_floor_applies()` is then
called with `recomputed_severity` as the authoritative severity — the raw `verdict.severity` is NEVER used
for floor evaluation. This closes P10-001: the pre-v1.13 hard floor keyed on the LLM-supplied
`verdict.severity`, so an attacker-influenced LLM could write `verdict.severity="LOW"` for a genuinely
CRITICAL native value and bypass the HIGH/CRITICAL floor with all enum checks passing. STEP 1a relocates
the severity trust from the LLM to the hook (O6), making the severity dimension of the floor genuinely
un-bypassable. Paired mutant **SM-44** (revert STEP 1a → trust `verdict.severity` directly) is killed by
VP-HOOK-030's per-family under-report vectors. This is NOT a second normalization — it is a CROSS-CHECK: the
LLM-reported severity must agree with what the deterministic table produces for the same inputs.
...
17-field + `validate_enums()` + STEP 1a SEVERITY-MISMATCH path** (REGARDLESS of any `investigation` substring in the path);
...
kill target; VP-HOOK-028's canonical-path leg asserts `.../verdict-alert-001.json` → 17-field path
...
hard-floor keys on the LIVE 17-field schema (**v1.13/P10-001: on the hook-RECOMPUTED severity from STEP 1a,
not raw `verdict.severity`**), so the corrected legs inject:
...
`ticket_id` are in neither the 17-field presence check nor `validate_enums()`, the verdict passed every prior
...
re-target/renumber this pass.** All 39
...
| SM-17 | disposition-guard 17-field `has()` list | drop one of the original 12 fields (e.g. `timeline_events`) from the presence list | VP-HOOK-025 per-field missing-field tests |
...
| SM-20 | disposition-guard 17-field `has()` list (new fields) | **severity-field-drop** — drop `severity`/`asset_type`/`ticket_action_type` (fields 13/14/15) from the presence list | VP-HOOK-025 missing-severity / missing-asset_type / missing-ticket_action_type tests (EC-010) |
...
**New mutation vector count: 39 (SM-9 … SM-45; SM-32 = SM-32a + SM-32b + SM-32-ext as of v1.9; SM-32a re-targeted + SM-32-ext kill vector re-worded at v1.10; +SM-38/SM-39/SM-40 at v1.10; +SM-41/SM-42 at v1.11; +SM-43 at v1.12; +SM-44/SM-45 at v1.13).** SM-9..SM-16,
...
With SM-9..SM-16 + SM-21..SM-22 + SM-28 + SM-29..SM-30 + **SM-31, SM-32a, SM-32b, SM-32-ext, SM-33..SM-45**
KILLED, the marker/authorization path meets the **≥95% require-review** target (SM-31/32a/32b/32-ext/33/34/38/39/41/**44/45**
land squarely on the disposition-guard emitter authorization boundary — **SM-44 on STEP 1a, SM-45 on WRITE_MARKER review-path fail-closed**; SM-35/36/37/40/42/43 on the require-review
...
- **VP-HOOK-025 12-vs-15 artifact-class split + confidence float→enum legs (NEW v1.3,
  ...
  still allow, no wrong-class 17-field deny), while the verdict-JSON class checks all 17. Mutant SM-30
  (artifact-class-over-strict — apply the 17-field set to investigation markdown) dies on the
  investigation-12-field-accept test — this guards the ADV-F2-P3-003 regression where 17-field
...
- **VP-HOOK-028 JSON-first canonical-path dispatch (extended v1.5, ADV-F2-P4-001 CRITICAL).** The pass-3
  ...
  (JSON-content/`.json` → 17-field verdict path REGARDLESS of the `investigation` substring; `*investigation-*.md`
  → 12-field; else fast-path) as an explicit VP-HOOK-028 regression leg: `.../verdict-alert-001.json` →
  17-field path → marker (positive, NOT misrouted); `.../investigation-001.md` → 12-field. Mutant SM-34
...
- **VP-HOOK-029 end-to-end consumer-boundary fail-loud invariant (NEW v1.5; RE-SCOPED v1.8/v1.9; re-scoped
  ...
  **[EXTENDED v1.11 — STEP-3 correctly-labeled-yet-unbindable deny, ADV-F2-P8-001 CRITICAL / D-DEC-012
  ...
  `jira_project_key`/`ticket_id` are in neither the 17-field presence check nor `validate_enums()`, the
...
- **VP-HOOK-024 consumer STEP 6a anti-fungibility (NEW v1.9, ADV-F2-P6-001 CRITICAL).** The adversary showed
  ...
  **[EXTENDED v1.11 — QUOTE-AWARE tokenizer, ADV-F2-P8-002 MAJOR / D-DEC-001.]** The adversary (P8-002) showed
  ...
  whitespace split of EC-024's own example — `jr issue create --project PRISM-DEMO --summary "rule matched
  ...
  **[P8-003 MINOR — step-6a is the SOLE anti-fungibility enforcement point.]** The adversary corrected a false
  ...
  tail-anchored: for `jr issue create --project PRISM-DEMO --label REVIEW-REQUIRED …` the regex matches the
  prefix up to the space after `PRISM-DEMO`, so **step 5 PASSES**. `( |$)` guards ONLY against project-KEY
  ...
  **[EXTENDED v1.12 — ESCAPED-QUOTE differential-vs-bash partition (O5), ADV-F2-P9-001 MAJOR / D-DEC-001 v1.12.]**
  ...
  the backslash-escape class. For the Iron-Law-compliant command `jr issue create --project PRISM-DEMO --summary
...
/ P10-003 WRITE_MARKER / P10-006 carve-out** propagation is a PO/architect-owned text alignment consistent with the
architecture-delta v1.13 body — the schema went 15→**17 fields**, a deliberate P10-001 schema extension, not a drift).
All delta BCs are otherwise
internally consistent with the finalized **33-VP** set (VP-SKILL 001–075, VP-HOOK 024–030; v1.10/v1.11/v1.12 add no
new VP — VP-HOOK-029 extended in place, VP-HOOK-024 / VP-SKILL-074 extended; **v1.13 adds VP-HOOK-030 + VP-SKILL-075
and extends VP-HOOK-029 / upgrades VP-SKILL-059 / renames VP-SKILL-064**)
and the SM-N catalog (SM-9..SM-45, **39 mutants** with SM-32 = SM-32a + SM-32b + SM-32-ext; +SM-38/SM-39/SM-40 at v1.10; +SM-41/SM-42 at v1.11; +SM-43 at v1.12; **+SM-44/SM-45 at v1.13**).
...
*F2 Verification Delta v1.13 complete. **33 VPs** (0 collisions, 0 renumbering): VP-SKILL 001–075, VP-HOOK
