   ...
   **Marker-validation algorithm (D-DEC-001 v2.0 — [UPDATED v1.14] iterative-consume):**
   ...
   for each file F in marker_dir/*.marker.json:
     ...
     (6) authorized_operations scope check — EXACT-TYPE MATCHING (ADV-F2-P6-001):
         has_review_label = structural_label_check(command)   # P8-002: quote-aware state-machine tokenizer; P7-005: token-position check, not substring
         ...
         #
         # ADV-F2-P7-005 FIX — STRUCTURAL TOKEN DETECTION; P8-002 UPDATE — QUOTE-AWARE TOKENIZER:
         ...
         #
         # P8-002 QUOTE-AWARE TOKENIZER (state machine):
         ...
         # The state machine accumulates all chars inside "..." or '...' into the current token body.
         #
         ...
         #       return True
         #   return False
         # EC-024 RECONCILIATION: for `jr issue create --project PRISM-DEMO --summary "rule --label REVIEW-REQUIRED in payload"`:
         ...
         has_review_label = structural_label_check(command)   # P8-002: quote-aware state-machine tokenizer (UNQUOTED/IN_SINGLE/IN_DOUBLE); P7-005: token-position check, not substring
