...
| **VP-SKILL-065** *(NEW v1.1; RE-SCOPED v1.9)* | BC-10.01.001 (Inv#11 — process-gap, ADV-F2-019; carve-out ADV-F2-P6-003) | none (next free = 065) | **RE-SCOPED — PROPOSED** (autonomy_enabled kill switch, narrowed v1.9 to zero REGULAR jr writes; create-review/comment-review escalation writes for genuine hard-floor verdicts EXEMPT per D-DEC-012 Option A — no longer silently FINALIZED, re-marked PROPOSED pending the new vector set) |
...
| **VP-HOOK-029** *(NEW v1.5; RE-SCOPED v1.8; FINALIZED v1.9)* | BC-10.01.001 (D-DEC-012 fail-loud, Inv#10 narrowed + STEP 4 under-label upgrade — ADV-F2-P5-001/P6-002); enforced by BC-3.03.001 (STEP 4 emit / error artifact) + BC-3.01.001 (consume create-review/comment-review) | none (max prior = 028; re-scope, not new ID) | **FINALIZED** (P0 fail-loud UNDER-LABEL invariant + kill-switch-on under-label vectors; VP-INDEX lifecycle PROPOSED → **FINALIZED** v1.9 per architecture-delta §8.15 item 1 / P6-010) |
...
**Result: ZERO namespace collisions. No renumbering required (two architect-proposed IDs corrected
to free slots before allocation — VP-SKILL-072→074, SM-33/34→36/37).** Each VP-SKILL-050..074
and VP-HOOK-024..029 appears in exactly one *owning* BC.

**VP-HOOK-029 namespace justification (VP-HOOK vs VP-SKILL — pass 4; scope corrected pass-5; FINALIZED pass-6).**
...
the STEP 4 under-label upgrade (verdict carries a NON-review token but `hard_floor_applies()`=true,
...
The "never silently discarded" guarantee is a hook-side emit obligation (BC-3.03.001 STEP 3+STEP 4)
+ consumer acceptance of the two review tokens (BC-3.01.001 v1.18 step 6/6a); BC-10.01.001 Inv#10
(narrowed at v1.8 so hard floors set `create-review`/`comment-review`, not `none`, and STEP 4
fail-loud-upgrades under-labeled hard-floor verdicts) is the authoritative *definition*.
**Pass-6 (ADV-F2-P6-002 CRITICAL / P6-010):** the architect's v1.9 §B STEP REORDER moves the
under-label upgrade to **STEP 4 (before the STEP 5 kill switch)** so an under-labeled hard-floor
verdict is upgraded to a review marker (or error+deny) **regardless of `autonomy_enabled`** — closing
the P6-002 silent-discard-under-kill-switch hole. VP-HOOK-029 is **FINALIZED v1.9** (P0) with the
added kill-switch-on under-label vectors (§2), per architecture-delta §8.15 item 1.
Enforcement is 100% hook-side → **VP-HOOK**. **VP-SKILL-072
...
| **VP-HOOK-029** *(NEW v1.5; RE-SCOPED v1.8; **FINALIZED v1.9**, **P0**)* | **Fail-loud UNDER-LABEL invariant (D-DEC-012 / ADV-F2-P5-001 CRITICAL / ADV-F2-P6-002 CRITICAL — re-scoped from the pass-4 happy-path-only scope; FINALIZED v1.9):** a hard-floor / Indeterminate / silent-sensor verdict that arrives carrying a **NON-review** `ticket_action_type ∈ {comment, create, assign, none}` MUST result in **exactly one** of {(a) a `create-review`/`comment-review` restricted marker in the store (the STEP 4 deterministic upgrade), (b) an explicit error artifact in `audit.log` AND a deny (the no-`jira_project_key` create path)} — and NEVER a silent allow-without-marker, **regardless of `autonomy_enabled`**. The pass-4 scope ("hard-floor WITH a review token → marker OR error") tested only the correct-label happy path and structurally could NOT detect the P5-001 silent-discard case (`action=comment/none` falling through the old STEP 5 to `emit allow without marker; RETURN`); the re-scope injects hard-floor verdicts with NON-review tokens and asserts the review-marker XOR error+deny outcome. **v1.9 (ADV-F2-P6-002 CRITICAL / P6-010): the STEP REORDER moves the under-label upgrade to STEP 4, which now runs BEFORE the STEP 5 `autonomy_enabled` kill switch — so an under-labeled hard-floor verdict is upgraded even when `autonomy_enabled=false` (closing the P6-002 silent-discard-under-kill-switch hole). VP-HOOK-029 is FINALIZED (P0) this pass.** This closes the D-DEC-008 regression where a hard-floor verdict labeled `ticket_action_type=none` (or comment/create/assign) got no marker, no Jira surface, and no error → the `[REVIEW-REQUIRED]`/`[BLIND-SPOT]` finding was silently dropped in unattended cron (no human to click approve). Enforced deterministically at the hook (STEP 4), NOT delegated to the trusted-LLM SKILL.md layer. | B-BEH + B-INT-XH | disposition-guard.sh on hard-floor verdicts carrying NON-review tokens — **STEP 4 branch (b) ticket_id present → comment-review upgrade:** `disposition=Indeterminate` (or HIGH severity / silent sensor) + `ticket_action_type=comment` + `ticket_id="SEC-123"` → assert a `comment-review` marker (`authorized_operations=["comment-review"]`, `command_pattern` anchored to `SEC-123`) exists AND an `UNDER-LABEL-CORRECTED` audit line is written; **STEP 4 branch (a) ticket_id null + jira_project_key present → create-review upgrade:** HIGH-severity + `ticket_action_type=create` + `ticket_id=null` + `jira_project_key="ORG_A"` → assert a `create-review` marker (`authorized_operations=["create-review"]`, `command_pattern` = `^jr (--output json )?issue create --project ORG_A --label (REVIEW-REQUIRED\|BLIND-SPOT)( \|$)`) exists + `UNDER-LABEL-CORRECTED` audit; also `Indeterminate` + `ticket_action_type=none` + `ticket_id=null` + project_key present → create-review marker; **STEP 4 branch (c) ticket_id null + project_key absent → error+deny:** `Indeterminate` + `ticket_action_type=create` (or `none`) + `ticket_id=null` + `jira_project_key` absent/empty → assert a `FAIL-LOUD` error artifact in `audit.log` AND require-review/emitter **deny** (NO marker); **v1.9 KILL-SWITCH-ON under-label vectors (P6-002 — STEP 4 upgrade fires BEFORE the STEP 5 kill switch): `autonomy_enabled=false` + (i) Indeterminate + `comment` + ticket_id present → comment-review marker STILL emitted (kill-switch EXEMPT via STEP 4 before STEP 5); (ii) HIGH-severity + `create` + ticket_id null + project_key present → create-review marker STILL emitted; (iii) Indeterminate + `none` + ticket_id null + project_key absent → error artifact + deny (NOT a silent allow); all three assert review-marker XOR error, NEVER silent allow, with the kill switch ON;** **fail-loud invariant assertion (all branches): assert exactly one of {review marker exists, error-artifact+deny} — assert NOT (empty marker dir AND no error) and NOT (silent allow-without-marker)**; **RETAINED correct-label happy-path vectors:** `create-review` token on a genuinely-hard-floor verdict → review marker in store (existing path unbroken); downstream require-review.sh on the matching `jr issue create --project ORG_A --label BLIND-SPOT`/`comment` → assert **allow** (review marker consumed, STEP 6a label-match passes). Shared `CLAUDE_PLUGIN_DATA=$(mktemp -d)`. **Paired mutants: SM-32a (remove STEP 4 under-label upgrade) killed by the under-label vectors; SM-32-ext (revert the STEP 4/5 order → kill switch before under-label upgrade) killed by the v1.9 kill-switch-ON under-label vectors (a silent allow with an empty marker dir under `autonomy_enabled=false` re-appears and the vectors catch it).** | BC-10.01.001 (v1.10 Inv#10 narrowed + STEP 4 fail-loud, D-DEC-012); enforced by BC-3.03.001 (v1.14 STEP 4 emit review marker / error artifact) + BC-3.01.001 (v1.18 consume create-review/comment-review + STEP 6a label match) |
| **VP-SKILL-072** *(NEW v1.5)* | **First-run 24h lookback correctness (ADV-F2-P4-012 — BC-10.01.001 Inv#13 / EC-001 / D-DEC-002):** when NO watermark file exists for an org×sensor pair (first invocation), the loop's Stage-1 query is bounded to `WHERE _time >= now() - INTERVAL 24 HOURS` (never a full-history scan), and after a successful run the watermark is persisted to the most-recent processed event `_time`. Distinct from VP-SKILL-050 (monotonicity on an EXISTING watermark). | B-INT | Run loop stub with an EMPTY `CLAUDE_PLUGIN_DATA/watermarks/` dir (no file for org×sensor); assert the emitted PrismQL carries the `now() - INTERVAL 24 HOURS` lower bound (and NOT an unbounded / full-history query); after run, assert a watermark file is persisted at the latest processed `_time`; control: pre-existing watermark → assert the 24h-lookback branch is NOT taken (query uses the watermark bound) | BC-10.01.001 (v1.10 Inv#13, EC-001, D-DEC-002) |
| **VP-SKILL-073** *(NEW v1.9, **P1**)* | **Late-event drop detection (ADV-F2-P6-007 — D-DEC-002 / BC-10.01.001 Stage-1):** when an ingested event's normalized `_time` is older than `watermark − WATERMARK_GRACE_SECONDS` (i.e. it arrived in-window this run but would fall outside the window next run), `DETECT_LATE_EVENT()` emits a `LATE_EVENT_DETECTED` audit entry to `${CLAUDE_PLUGIN_DATA}/watermarks/audit.log`; the event is **NOT dropped** — it proceeds to the VALIDATE stage and is processed normally. This converts the D-DEC-002 grace-window permanent-drop trade-off from a SILENT data-loss path into an AUDITABLE one (operators tune `WATERMARK_GRACE_SECONDS` from the log). Distinct from VP-SKILL-068 (in-grace dedup) and VP-SKILL-050 (monotonicity). | B-INT | Run loop stub with a pre-existing watermark; inject an OCSF event whose normalized `_time < watermark − WATERMARK_GRACE_SECONDS`; assert a `LATE_EVENT_DETECTED` line is present in `watermarks/audit.log` (with `event_time=`, `watermark=`, `grace_window=`) AND the event still reaches VALIDATE (not silently discarded); control leg: an event with `_time ≥ watermark − GRACE` → NO `LATE_EVENT_DETECTED` line; first-run (no watermark) → `DETECT_LATE_EVENT` returns early, no false positive | BC-10.01.001 (v1.10 Stage-1, D-DEC-002 late-event fail-loud) |
| **VP-SKILL-074** *(NEW v1.9, **P1**; namespace correction — architect §8.15 item 3 said "VP-SKILL-072", occupied)* | **Severity normalization correctness (ADV-F2-P6-005 — D-DEC-013 / BC-10.01.001 Stage-1/Stage-5):** `NORMALIZE_SEVERITY(native_severity, sensor_family)` produces ONLY members of `{LOW,MEDIUM,HIGH,CRITICAL}` (case-exact, so the downstream `validate_enums()` gate never fail-closed-denies a well-normalized verdict); each sensor family maps per the D-DEC-013 table (CrowdStrike numeric 1-100 boundaries 26/51/76; NVD/CVSS float boundaries 4.0/7.0/9.0; Armis/Claroty risk bands 1:1 case-fold, Armis Informational→LOW); an UNRECOGNIZED native value (any family, incl. Cyberint pending ASM-008) → `CRITICAL` WITH `uncertainty_explicit` appended (an AUDITABLE conservative default — NEVER a silent enum-deny, and NEVER a silent LOW). Applied at Stage 1 INGEST and re-applied at Stage 5 SCORE (and Stage 1 known-FP fast-path). | B-INT (boundary/equivalence-partition) | boundary fixtures per family — CrowdStrike `{25→LOW, 26→MEDIUM, 50→MEDIUM, 51→HIGH, 75→HIGH, 76→CRITICAL}`; CVSS `{3.9→LOW, 4.0→MEDIUM, 6.9→MEDIUM, 7.0→HIGH, 8.9→HIGH, 9.0→CRITICAL}`; Armis `{Critical→CRITICAL, Informational→LOW}` case-fold; assert the emitted `verdict.severity` ∈ the four-value enum (case-exact); **unrecognized fixture (e.g. CrowdStrike `"Sev5"`, Cyberint any) → assert `verdict.severity=="CRITICAL"` AND `verdict.uncertainty_explicit` is non-null and contains the "Unrecognized severity" annotation** (auditable, not a deny); assert NO raw sensor-native string reaches `validate_enums()` | BC-10.01.001 (v1.10 Stage-1/Stage-5 field 13, D-DEC-013) |
...
(VP-SKILL-069, 070, 071, VP-HOOK-028) + 2 v1.5 VPs (VP-HOOK-029, VP-SKILL-072) + **2 NEW v1.9 VPs
...
027/028/029 are cross-hook B-INT-XH; **VP-HOOK-029 is P0 fail-loud, FINALIZED v1.9 with kill-switch-on
under-label vectors**), **23 skill properties** (VP-SKILL-073 is a late-event-detection integration
test and VP-SKILL-074 a severity-normalization boundary/equivalence test — both NEW v1.9, PROPOSED;
...
review-exempt carve-out, re-marked PROPOSED**). **VP-HOOK-029 lifecycle: tagged P1 at pass-4, re-scoped
to the P5-001 CRITICAL under-label vectors at pass-5 (fix-priority P0), and FINALIZED (PROPOSED →
FINALIZED) at pass-6 per architecture-delta §8.15 item 1 / P6-010 with the added `autonomy_enabled=false`
kill-switch-on under-label vectors — the convergence-blocking safety invariant is no longer deferred to
F6. VP-SKILL-073/074 and the re-scoped VP-SKILL-065 carry VP-INDEX lifecycle PROPOSED (adjudicated in
F6).**
...
AND the kill switch (emitter Step 3 correct-label + Step 4 under-label upgrade both run before the
Step 5 kill switch — post ADV-F2-P6-002 reorder), because creating a `[REVIEW-REQUIRED]` /
...
**[UPDATED v1.8 / v1.9 — the STEP 3 review-exemption is GATED on `hard_floor_applies()`; the review token is
no longer trusted unconditionally (ADV-F2-P5-002 MAJOR); the STEP 4 under-label upgrade is the
fail-loud dual (ADV-F2-P5-001 CRITICAL) and — post the ADV-F2-P6-002 reorder — now runs BEFORE the
STEP 5 kill switch.]** Pass-4 treated the `create-review`/`comment-review`
...
token) is enforced at STEP 4 and proved by **VP-HOOK-029**; its paired mutant is **SM-32a** (remove the
STEP 4 upgrade → silent discard), with **SM-32-ext** (revert the STEP 4/5 order) killed by VP-HOOK-029's
v1.9 kill-switch-on under-label vectors. The unconditional-hard-floor legs (1)–(6) above are UNCHANGED for
...
"SM-33/SM-34", which collide with occupied ids; they are allocated the next free SM-36/SM-37).** All 31
...
| SM-21 | disposition-guard hard-floor key | replace `verdict.severity` with `verdict.confidence` as the severity proxy (**ADV-F2-001 latent bypass**: HIGH-severity + low-confidence escapes the floor) | VP-HOOK-026 leg (2): `severity=HIGH` + `confidence=low` → assert NO regular (comment/create/assign-scoped) marker (HIGH severity is hard-floor by severity; the STEP 4 upgrade may place a REVIEW marker instead) — the mutant keys on `confidence=low`, mis-classifies as non-hard-floor, issues a REGULAR marker, and dies |
...
| SM-32a | disposition-guard emitter STEP 4 under-label upgrade *(renumbered from STEP 5 by the ADV-F2-P6-002 reorder)* | **step4-under-label-upgrade-removed** (ADV-F2-P5-001 CRITICAL) — revert STEP 4 to the pre-fix "IF action=='none' OR hard_floor_applies(): emit allow without marker; RETURN" so a hard-floor / Indeterminate / silent-sensor verdict carrying a NON-review `ticket_action_type ∈ {comment,create,assign,none}` gets NO marker, NO Jira surface, and NO error → the finding is silently dropped (the P5-001 silent-discard) | **VP-HOOK-029 under-label vectors** (Indeterminate+comment+ticket_id present → comment-review marker; HIGH+create+ticket_id null+project_key present → create-review marker; Indeterminate+none+ticket_id null+project_key absent → error+deny; the fail-loud "review-marker XOR error+deny, never silent allow" assertion) — the mutant yields an empty marker dir + no error and dies |
| SM-32-ext | disposition-guard emitter STEP order (kill switch vs under-label upgrade) | **step-order-reverted** (ADV-F2-P6-002 CRITICAL) — revert the ADV-F2-P6-002 reorder so the `autonomy_enabled` kill switch runs BEFORE the hard-floor under-label upgrade (kill switch back at the old STEP 4, upgrade back at the old STEP 5). Under `autonomy_enabled=false`, an under-labeled hard-floor verdict then hits the kill switch first → `emit allow without marker; RETURN` before the upgrade can fire → the finding is silently discarded exactly as in P6-002 | **VP-HOOK-029 kill-switch-ON under-label vectors** (`autonomy_enabled=false` + under-labeled hard-floor → review marker XOR error+deny, never silent allow) — under the mutant the marker dir is empty with no error under the kill switch, and the vectors catch it |
...
**New mutation vector count: 31 (SM-9 … SM-37; SM-32 = SM-32a + SM-32b + SM-32-ext as of v1.9).** SM-9..SM-16,
...
**v1.9 (pass-6): SM-32-ext is the ADV-F2-P6-002 CRITICAL step-order-reverted sentinel (killed by
VP-HOOK-029's kill-switch-on under-label vectors — proving the STEP 4 under-label upgrade runs before
the STEP 5 kill switch); SM-36 and SM-37 are the ADV-F2-P6-001 CRITICAL consumer anti-fungibility
sentinels (killed by VP-HOOK-024's STEP 6a exact-type vectors in both directions) — the fungibility
of review vs regular create markers was the pass-6 kill-switch/hard-floor bypass root cause.** With
SM-9..SM-16 + SM-21..SM-22 + SM-28 + SM-29..SM-30 + **SM-31, SM-32a, SM-32b, SM-32-ext, SM-33..SM-37**
KILLED, the marker/authorization path meets the **≥95% require-review** target (SM-31/32a/32b/32-ext/33/34
land squarely on the disposition-guard emitter authorization boundary; SM-35/36/37 on the require-review
...
| BC-10.01.001 (v1.10) | VP-HOOK-026, VP-HOOK-027, VP-HOOK-028, **VP-HOOK-029**, VP-SKILL-050, **072**, **073**, **074**, 060, 061, 062, 063, **064**, **065**, **068** | 75 — hard-floor{Indeterminate, severity=HIGH, critical-asset, **unknown-asset+LOW+benign (1, VP-HOOK-026, ADV-F2-P3-001)**, T1003, degraded-sensor}=6, **v1.5 review-surfacing (hard_floor_applies()=TRUE positive controls){Indeterminate+create-review→marker=1, silent+comment-review→marker=1, HIGH+create-review→marker=1, authorized_operations=['create-review']=1 (VP-HOOK-026, D-DEC-012)}=4**, **v1.8 over-label gate (hard_floor_applies()=FALSE){non-hard-floor-TP+create-review→no-marker=1, non-hard-floor-FP+comment-review+autonomy=false→no-marker=1, LOW-standard+create-review→no-marker=1 (VP-HOOK-026, ADV-F2-P5-002, SM-32b kill)}=3**, **v1.5 kill-switch-determinism (STEP 5 post-reorder){autonomy=false+create→no-marker=1, +comment→no-marker=1, autonomy-absent→false→no-marker=1, autonomy=false+create-review→marker-still (exempt)=1 (VP-HOOK-026, P4-005, SM-33 kill)}=4**, **v1.8 fail-loud under-label (STEP 4 upgrade){comment-review-upgrade (ticket_id present)=1, create-review-upgrade (ticket_id null + project_key)=1, error+deny (project_key absent)=1, UNDER-LABEL-CORRECTED-audit=1, downstream jr review→allow=1, retained correct-label happy-path=1 (VP-HOOK-029, ADV-F2-P5-001, SM-32a kill)}=6**, **v1.9 kill-switch-ON under-label (STEP 4 before STEP 5, P6-002){autonomy=false+Indeterminate+comment+ticket_id→comment-review-marker-still=1, autonomy=false+HIGH+create+ticket_id-null+project_key→create-review-marker-still=1, autonomy=false+Indeterminate+none+no-project_key→error+deny=1 (VP-HOOK-029, ADV-F2-P6-002, SM-32-ext kill)}=3**, verdict-path-reachability{non-verdict-path→no-marker→jr-deny=1, verdict-path→marker→jr-allow control=1 (VP-HOOK-028)}=2, **v1.5 canonical-dispatch{investigations/verdict-*.json→15-field→marker (JSON-first)=1, investigation-*.md→12-field=1 (VP-HOOK-028, P4-001, SM-34 kill)}=2**, watermark{monotonic,future-reject}=2, **v1.5 first-run{no-watermark→24h-lookback-query=1, post-run-persist=1, existing-watermark→no-lookback control=1 (VP-SKILL-072, P4-012)}=3**, **v1.9 late-event{_time<watermark−GRACE→LATE_EVENT_DETECTED-audit+still-processed=1, in-window→no-detection control=1, first-run→no-false-positive=1 (VP-SKILL-073, P6-007)}=3**, **v1.9 severity-normalization{CrowdStrike 25→LOW/26→MEDIUM/76→CRITICAL=3, CVSS 3.9→LOW/9.0→CRITICAL=2, Armis Critical→CRITICAL+Informational→LOW case-fold=1, unrecognized→CRITICAL+uncertainty_explicit (auditable, not deny)=1 (VP-SKILL-074, P6-005, D-DEC-013)}=7**, known-FP-before-enrich=2, **hard-floor-before-known-FP-autoclose (EC-009)=1**, BLIND-SPOT positive=2, never-reopen-closed=2, Tavily-degrade=2, `--bare`-absent wrapper assertion=1, allowlist-matches-SKILL=1, cross-hook ASM-009 integration=2, **org_slug{cross-tenant-org-a≠b/c=2, static query-construction check=1, unscoped-query adversarial=1}=4 (VP-SKILL-064)**, **kill-switch (RE-SCOPED v1.9, Option A carve-out){zero-REGULAR-markers=1, zero-REGULAR-jr-writes=1, evidence+draft-still-allowed=1, silent-sensor→create-review-jr-write-STILL-executes (exempt)=1}=4 (VP-SKILL-065)**, **stage-order{document-before-action positive=1, jr-before-Write→deny=1, TTL-expiry→deny=1}=3 (VP-HOOK-027)**, **grace-window-dedup{in-grace+existing-open-ticket→comment=1, in-grace+no-ticket→create=1, _time-normalize+boundary=1, dedup-off→double-ticket mutant kill=1}=4 (VP-SKILL-068)** | ~24 | monitoring-loop CRITICAL; +14 over v1.8 for VP-HOOK-029 kill-switch-ON under-label (3, P6-002/SM-32-ext) + VP-SKILL-065 Option-A carve-out re-scope (+1) + VP-SKILL-073 late-event (3, P6-007) + VP-SKILL-074 severity normalization (7, P6-005) |

**Estimated new BATS test cases: ~178** (hooks/skills/integration; was ~162 at v1.8) + **~80 parity (.ps1)**
additions ≈ **~258 new test cases** for the cycle (was ~238). The v1.9 pass-6 remediation adds **+16 BATS**:
**+2 BC-3.01.001** (consumer STEP 6a anti-fungibility both directions — P6-001 CRITICAL, SM-36/SM-37
kill) and **+14 BC-10.01.001** (VP-HOOK-029 kill-switch-ON under-label +3 — P6-002 CRITICAL, SM-32-ext
kill; VP-SKILL-065 Option-A carve-out re-scope +1; VP-SKILL-073 late-event +3 — P6-007; VP-SKILL-074
severity normalization +7 — P6-005/D-DEC-013 boundary partitions). Plus **~4 parity** (the consumer
STEP 6a check and the disposition-guard STEP-reorder each need a `.ps1` sibling per CONV-004; the
VP-SKILL-073/074 loop-stub + normalization integration adds little parity). The prior v1.8 pass-5
re-scope added +7 BATS on BC-10.01.001 (VP-HOOK-026 over-label gate +3; VP-HOOK-029 fail-loud under-label
+4). The v1.9 mutants (SM-32-ext, SM-36, SM-37) all land on the CRITICAL disposition-guard/require-review
authorization path (STEP order + consumer anti-fungibility), so these remain near-exhaustive
input-partition BATS on the marker path. This is consistent with the
...
- **VP-SKILL-065 (autonomy_enabled kill switch — Inv#11) — RE-SCOPED v1.9 (ADV-F2-P6-003 MAJOR / D-DEC-012
  ...
  jr-mock spy DOES record the review escalation write (kill-switch EXEMPT via the STEP 4 upgrade / STEP 3
  correct-label path, both before the STEP 5 kill switch). This is the positive companion to VP-HOOK-026
...
- **VP-HOOK-026 unknown-asset leg (NEW v1.3, ADV-F2-P3-001 CRITICAL)** extends the existing
  ...
  REVIEW marker from the STEP 4 under-label upgrade is acceptable (its fail-loud completeness is owned by
...
- **VP-HOOK-026 review-surfacing + kill-switch determinism (extended v1.5, D-DEC-012 / ADV-F2-P4-004 /
  ...
  kill switch (emitter Step 3 correct-label + Step 4 under-label upgrade both run before the Step 5 kill
...
- **VP-HOOK-029 fail-loud invariant (NEW v1.5; RE-SCOPED v1.8 to the UNDER-LABEL vectors; FINALIZED v1.9, P0,
  D-DEC-012 / ADV-F2-P5-001 CRITICAL / ADV-F2-P6-002 CRITICAL).** This is the closure of the mutual exclusion
  the adversary flagged: BC-10.01.001 v1.7 Inv#10 forced hard-floor verdicts to `ticket_action_type=none`
  (no marker), while EC-006/EC-014 required the loop to create `[BLIND-SPOT]` / `[REVIEW-REQUIRED]` tickets
  — so in unattended cron a hard-floor verdict was silently dropped (no marker → require-review denies create
  ...
  (`comment`/`create`/`assign`/`none`) fell through the pre-fix upgrade step (`IF action==none OR
  hard_floor_applies(): emit allow without marker; RETURN`) with no marker, no Jira surface, no error. The
  architect's D-DEC-008 fix deterministically UPGRADES an under-labeled hard-floor verdict at **STEP 4**
  ...
  error artifact and denies. **Pass-6 root cause + fix (P6-002 CRITICAL):** the pass-5 vectors implicitly
  assumed the upgrade step was reachable, but in v1.8 it ran as STEP 5 AFTER the STEP 4 `autonomy_enabled`
  kill switch — so `under-label + autonomy_enabled=false` hit the kill switch first and the finding vanished
  silently (no marker, no error, no audit), and VP-HOOK-029 had NO vector for that case. The architect's v1.9
  §B STEP REORDER moves the under-label upgrade to **STEP 4 (before the STEP 5 kill switch)**, and this pass
  **adds the three kill-switch-ON under-label vectors** — `autonomy_enabled=false` + under-labeled hard-floor
  → (i) comment-review marker (ticket_id present), (ii) create-review marker (ticket_id null + project_key),
  (iii) error+deny (no project_key) — each asserting review-marker XOR error, NEVER silent allow, with the
  kill switch ON. **Re-scoped/expanded VP-HOOK-029 injects hard-floor verdicts carrying NON-review tokens and
  asserts EXACTLY ONE of {review-marker emitted, error-artifact+deny} across all STEP 4 branches (both
  `autonomy_enabled=true` and `=false`) — assert NOT (empty marker dir AND no error) and NOT (silent
  allow-without-marker)**; the correct-label happy-path vectors are RETAINED. Reuses the ASM-009 cross-hook
  harness (disposition-guard emit → require-review consume of the review token + STEP 6a label match,
  BC-3.01.001 v1.18 step 6/6a). **FINALIZED (PROPOSED → FINALIZED) v1.9 per architecture-delta §8.15 item 1 /
  P6-010** — the convergence-blocking safety invariant is no longer deferred to F6. **Paired mutants: SM-32a
  (step4-under-label-upgrade-removed) killed by the under-label vectors; SM-32-ext (step-order-reverted →
  kill switch back before the upgrade) killed by the new kill-switch-ON under-label vectors; both disjoint
  from SM-32b (over-label, killed by VP-HOOK-026).**
...
**G. NEW corrections required by this v1.9 (pass-6: VP-HOOK-029 FINALIZE + kill-switch-on vectors,
consumer STEP 6a, VP-SKILL-065 re-scope, VP-SKILL-073 late-event, VP-SKILL-074 severity normalization,
SM-32-ext / SM-36 / SM-37).** The BODY updates — the disposition-guard STEP REORDER (STEP 4 under-label
upgrade before STEP 5 kill switch), the consumer STEP 6a anti-fungibility cross-check + the create-review
...
internally consistent with the finalized **31-VP** set (29 through v1.8 + **VP-SKILL-073** + **VP-SKILL-074**)
and the SM-N catalog (SM-9..SM-37, **31 mutants** with SM-32 = SM-32a + SM-32b + SM-32-ext as of v1.9).
...
*F2 Verification Delta v1.9 complete. **31 VPs** (0 collisions, 0 renumbering): VP-SKILL 001–074,
VP-HOOK 024–029. **Two independently-verified namespace corrections vs architect §8.15:** severity
normalization is **VP-SKILL-074** (§8.15 item 3's "VP-SKILL-072" collides with the occupied first-run
lookback VP); the consumer anti-fungibility mutants are **SM-36/SM-37** (§8.15 item 5's "SM-33/SM-34"
collide with occupied pass-4 mutants). **Pass-6 remediation (ADV-F2-P6-001/002/003/005/007/010,
architecture-delta v1.9 §8.15):** **(1) [P6-002 CRITICAL / P6-010]** **VP-HOOK-029 FINALIZED** (PROPOSED
→ FINALIZED, P0) with the kill-switch-on under-label vectors — the STEP REORDER puts the STEP 4 under-label
upgrade BEFORE the STEP 5 kill switch, so `autonomy_enabled=false` + under-labeled hard-floor still yields a
review marker (comment-review if ticket_id present; create-review if ticket_id null + project_key) XOR an
error+deny, NEVER a silent allow. **(2) [P6-001 CRITICAL]** consumer anti-fungibility vectors under
**VP-HOOK-024** — require-review STEP 6a exact-type matching in BOTH directions (create-review marker +
no-`--label` command → DENY; create marker + `--label` command → DENY; correct pairings consume; EC-023);
the create-review `command_pattern` now encodes `--label (REVIEW-REQUIRED|BLIND-SPOT)( |$)` in fixed
second position; hook-side label enforcement ADOPTED (D-DEC-012 reversal). **(3) [P6-003 MAJOR]**
**VP-SKILL-065 RE-SCOPED** (FINALIZED → PROPOSED) to zero-REGULAR-writes with the D-DEC-012 Option-A
carve-out (create-review/comment-review escalation writes for genuine hard-floor verdicts still execute
under the kill switch); jr-mock spy updated. **(4) [P6-005 MAJOR]** new **VP-SKILL-074** (severity
normalization, D-DEC-013 — per-sensor-family mapping at Stage 1/5; unrecognized → CRITICAL +
`uncertainty_explicit`, auditable, never a silent enum-deny; case-exact). **(5) [P6-007 MINOR]** new
**VP-SKILL-073** (late-event drop detection, D-DEC-002 — `_time < watermark − GRACE` → `LATE_EVENT_DETECTED`
audit, event still processed, never silent drop). **Mutants: SM-32-ext** (step-order-reverted; killed by
VP-HOOK-029 kill-switch-on vectors), **SM-36** (consumer-review-label-check-removed) and **SM-37**
(consumer-reverse-label-check-removed) — both killed by VP-HOOK-024's STEP 6a vectors. STEP 4/5 swap swept
through the SM catalog / VP rows / §3/§6 notes; "zero jr writes" phrasing re-scoped. **31 SM-N mutants
(SM-9..SM-37, SM-32 = SM-32a + SM-32b + SM-32-ext)** — all on the CRITICAL disposition-guard/require-review
authorization path (≥95%). **~178 new BATS + ~80 parity ≈ ~258** test cases estimated for F3 sizing (was
~238; +16 BATS — BC-3.01.001 anti-fungibility +2, BC-10.01.001 kill-switch-on +3 / VP-SKILL-065 +1 /
late-event +3 / severity normalization +7). Live-BC targets at v1.9 edit time (pass-6): **BC-3.01.001 v1.18,
BC-3.03.001 v1.15, BC-10.01.001 v1.11** (STEP reorder + consumer STEP 6a + Inv#11 carve-out + severity
normalization + late-event BODY owned by PO per architecture-delta §8.14); verification-delta made
internally self-consistent (VP table, mutant catalog, §3 discussion, §5 counts, §6 notes, §7). FV
cross-references routed to PO in §7 Part G. Cross-doc/other-file version-ref reconciliation deferred to the
dedicated version-coherence sweep (ADV-F2-P3-007/P3-009).*
