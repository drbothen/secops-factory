   ...
   **Marker-validation algorithm (D-DEC-001 v2.0 — [UPDATED v1.14] iterative-consume):**
   ...
   for each file F in marker_dir/*.marker.json:
     ...
     (6) authorized_operations scope check — EXACT-TYPE MATCHING (ADV-F2-P6-001):
         ...
         # VP-HOOK-024 cross-reference (v1.18, ADV-F2-P6-001): step (6a) anti-fungibility cross-check
         # extends VP-HOOK-024 with two new P6-001 vectors — valid marker consumed only when
         # label-type matches marker type in BOTH directions (EC-023).
