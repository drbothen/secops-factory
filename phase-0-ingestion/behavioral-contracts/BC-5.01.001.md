...
7. **[UPDATED v1.5; reconciled v1.9 P11-004; rewritten v1.10 P13-001; gate-1-first v1.11 P16-001; D-029 save-always-succeeds v1.13; P27-001/P27-002 structural-retain-infra-fail-loud v1.14]** Stage 7 includes a Jira action (comment, create, or review marker) to surface the investigation finding. **DI-013 RESOLVED (D-DEC-001):** The Jira action routing via the **Separate Human-Comment Marker Path** (BC-3.03.001 v1.36) follows the **D-029 SAVE-ALWAYS-SUCCEEDS** model. The investigation Write **always succeeds** for **ALL dispositions** (FP, TP, BTP, Indeterminate, or any other). **MUST NOT be denied:** now holds **structurally** under D-029 — no **disposition-/hard-floor-based** deny is possible on the markdown path for a **STRUCTURALLY COMPLETE** investigation (all 12 ICD-203 headings + Alternatives-Considered + charset validation passes); **structural ICD-203 completeness, Alternatives-Considered, and charset-injection guards are retained and can still deny (EC-004/EC-010 — pre-D-029-routing structural gates)**; AND **infrastructure marker-write failure on the review path (MARKER-WRITE-FAILED) fails loud** rather than silently dropping the escalation — infrastructure-failure denies are DISTINCT from D-029's content/disposition guarantee (P27-001/P27-002). **D-029 routing** (see BC-3.03.001 v1.36 Separate Human-Comment Marker Path for authoritative pseudocode): (a) FP + no hard floor (`hard_floor_triggered=false`) → **allow-without-marker** (Write succeeds; no Jira action authorized; subsequent `jr issue comment` falls to human permission-approval as before); (b) non-FP/PARSE_FAIL (any `autonomy_enabled` value, any `hard_floor_triggered` value) → **MARKDOWN_REVIEW_PATH** review marker (create-review or comment-review issued; `jr issue create --label REVIEW-REQUIRED` or `jr issue comment` is review-marker-gated via require-review — D-DEC-012 Option A EXEMPT regardless of `autonomy_enabled`); (c) FP + hard floor (`hard_floor_triggered=true`, e.g., Indeterminate disposition in content, forbidden technique T1003/T1068/T1021/T1041, degraded/silent sensor) → **MARKDOWN_REVIEW_PATH** (hard floor wins over FP disposition; review marker issued; D-029). `autonomy_enabled` is not a routing gate under D-029. **NOTE (P11-004):** Verdict-only fields (`scored_priority ∈ {HIGH,CRIT}` and critical/unknown `asset_type`) are NOT present in a 12-field investigation markdown and are NOT checked on this path — those floors apply only to the verdict-class path (BC-3.03.001 v1.34 PC#1). **Verification property consumed: VP-HOOK-031 (FINALIZED P0 per verification-delta (VP-HOOK-031, D-029/P27-001/P27-002 scope)) — this invariant is a consumer of the Separate Human-Comment Marker Path property; D-029 save-always-succeeds guarantees the investigation Write succeeds for all dispositions (structurally complete investigations only — P27-001); structural completeness and charset guards are pre-routing structural DENY gates (EC-004/EC-010) not covered by D-029; infrastructure marker-write failure on the review path (MARKER-WRITE-FAILED) is fail-closed (P27-002); non-FP dispositions route to MARKDOWN_REVIEW_PATH (review marker issued); FP + no hard floor → allow-without-marker; no disposition from a human analyst save yields an autonomous comment marker (P13-001 MARKDOWN_COMMENT_PATH ELIMINATED). Paired mutant SM-47 (markdown-routed-into-verdict-emitter) is the kill target for compliant-save-allowed and no-validate_enums-on-markdown vectors.**
   ...
   Confidence: D-DEC-001 binding decision (architecture-delta.md v1.1); BC-3.01.001 v1.22 PC#2 (marker-validation allow path); BC-3.03.001 v1.36 (Separate Human-Comment Marker Path — D-029 save-always-succeeds, P27-001/P27-002 structural-retain-infra-fail-loud, P22-001 / P13-001).
