...
| Total automated tests | **150** BATS `@test` cases (hooks 44, skills 81, integration 11, parity 14) — all pass locally. (hooks 32→44: +12 from PR #15 SEC-009/ADV-0-801 precedence-bypass + regression tests, on top of +9 from PR #13/#14.) |
| Tests that silently skip without `pwsh` (PowerShell parity) | **12 of 14** parity tests |
| Highest-value confirmed defect (still OPEN) | **disposition-guard false-pass** — anti-confirmation-bias gate is trivially defeated (re-confirmed live 2026-07-19: negating text containing the phrase still returns `allow`) |
| Resolved since original (verified) | **SEC-009 / ADV-0-801 allowlist-precedence bypass** (CRITICAL) → write-block now evaluated before allowlist (PR #15, d304fa5); **SM-3 fail-open** → fail-closed (PR #13, SEC-002); **DI-001 secrets** → gitignored (PR #12); **SM-2 assign/create** → mitigated by fail-closed default (PR #13) |
...
| Total automated tests | 150 `@test` cases | bats-core 1.13.0 |
| Test suites | 4 (`hooks.bats` 44, `skills.bats` 81, `integration.bats` 11, `parity.bats` 14) | bats-core |
...
Automated mutation tooling does not exist for bash-as-config. A manual mutation probe was run against the **6-hook mutation-testable set** (the 5 pure hooks + session-greeting's pure activation-gate sub-function) by reasoning about which source mutations the current 150-test suite would fail to kill (re-run post PR #15). Both **surviving mutants** (test-suite blind spots) and representative **killed mutants** (SM-8) were confirmed with live hook execution.
...
| SM-2 | require-review **write-block if-block** (delete `jr issue assign` / `jr issue create`) | delete `jr issue assign` / `jr issue create` from the write-block | **LARGELY MITIGATED (PR #13/#15).** With the fail-closed default in place and the write-block now evaluated first (PR #15), deleting these lines no longer changes behavior (unknown subcommand still denies) — the mutant is now near-equivalent. The SEC-002 test (`jr issue duplicate` → deny) guards the fail-closed default. Residual: still **no dedicated positive test** asserting assign/create deny (live-confirmed both deny 2026-07-19). Downgraded HIGH → LOW. | **LOW** (was HIGH) |
...
| GAP-2 | ~~require-review `assign`/`create`/fail-open untested + allowlist-precedence bypass~~ **RESOLVED (PR #13/#14/#15)** — fail-open → fail-closed (SEC-002); `comment` gated (SEC-001); allowlist expanded + tested; **CRITICAL allowlist-precedence bypass fixed (SEC-009/ADV-0-801, PR #15)** — write-block now first, bypass+regression tested. Residual (LOW): no dedicated `assign`/`create` positive test | require-review hook (write-block if-block; fail-closed catch-all), BC-3.01.001 | LOW (was CRITICAL) — write-block-first ordering + fail-closed default now backstop all verbs | 0.1 day (residual) |
...
- [x] Numerical baselines provided where tools apply (150 tests; per-BC coverage matrix; shellcheck clean; parity skip count)
