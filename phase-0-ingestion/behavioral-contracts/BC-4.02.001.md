...
7. **[NEW v1.4] Four §3.4 ticket-action types as distinct postconditions:**
   ...
   - **PC#7b — Link as related (§3.4 rule 2 — comment+link, D-022):** **[UPDATED v1.13 — P18-003/D-022]** If a ticket for the same asset or IOC exists but has a different root cause, the loop performs **two sequential Stage-7 verdict Writes (D-022 option b)**: verdict-1 (`ticket_action_type=comment`, `ticket_id=KEY1`) → comment marker issued → Stage 8 executes `jr issue comment KEY1 "..."` (marker consumed); verdict-2 (`ticket_action_type=link`, `ticket_id=KEY1`, `link_target_ticket_id=KEY2`) → link marker issued → Stage 8 executes `jr issue link KEY1 KEY2` (marker consumed, D-020). Each marker is single-use and anti-fungible — a comment marker does NOT authorize `jr issue link`, and a link marker does NOT authorize `jr issue comment`. Link-type is "Relates" (jr default; `--type` is never loop-influenceable per D-020 Iron Law). No ticket reopening occurs. Include §3.5 SLA surface statement.
...
| Existing open ticket (same asset, different root cause, §3.4 rule 2) | Two sequential Stage-7 verdict Writes (D-022): verdict-1 comment marker → `jr issue comment KEY1 "..."`; verdict-2 link marker (ticket_id=KEY1, link_target_ticket_id=KEY2) → `jr issue link KEY1 KEY2`; anti-fungibility: comment marker does NOT authorize link call; SLA surface statement emitted | edge-case (PC#7b) |
