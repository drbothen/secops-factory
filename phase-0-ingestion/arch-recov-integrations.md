...
| Perplexity MCP | MCP (stdio/JSON-RPC, npx-launched) | `PERPLEXITY_API_KEY` in `.mcp.json` (untracked) | CVE research, threat intel, OSINT sizing | Graceful fallback wired in every skill: on first Perplexity tool call failure → switch to WebSearch + WebFetch for session. All three research tiers (search/ask/reason/research) have explicit fallback paths. |
