...
function Invoke-ValidateMarkerForCommand([string]$Cmd) {
    [System.Console]::WriteLine("PS1_ENTRY cmd=[$Cmd] pluginData=[$($env:CLAUDE_PLUGIN_DATA)] ps1debug=[$($env:PS1_DEBUG)]")
    $pluginData = $env:CLAUDE_PLUGIN_DATA
    if ([string]::IsNullOrEmpty($pluginData)) { [System.Console]::WriteLine("PS1_BAIL empty-pluginData"); return $false }
    $markerDir = Join-Path $pluginData 'markers'
    if (-not (Test-Path $markerDir -PathType Container)) { [System.Console]::WriteLine("PS1_BAIL no-markerDir=[$markerDir]"); return $false }

    $debugMode = ($env:PS1_DEBUG -eq '1')
    if ($debugMode) { [System.Console]::WriteLine("DEBUG: pluginData=$pluginData markerDir=$markerDir dirExists=$(Test-Path $markerDir -PathType Container)") }
    ...
    if ($debugMode) { [System.Console]::WriteLine("DEBUG: markerFiles count=$(@($markerFiles).Count)") }
    [System.Console]::WriteLine("PS1_FILES count=$(@($markerFiles).Count) markerDir=[$markerDir]")
    foreach ($mf in $markerFiles) {
        ...
        if ($mf.DirectoryName -ne $markerDir) {
            if ($debugMode) { [System.Console]::WriteLine("DEBUG: path-mismatch dir=$($mf.DirectoryName) expected=$markerDir") }
            continue
        }
        ...
        catch { [System.Console]::WriteLine("PS1_PARSE_ERR file=[$($mf.Name)] err=[$($_.Exception.Message)]"); continue }
        ...
        if ($debugMode) { [System.Console]::WriteLine("DEBUG: file=$($mf.Name) issuedAt=$issuedAt nowTs=$nowTs iso8601ok=$(Test-Iso8601Utc $issuedAt) future=$([string]::CompareOrdinal($issuedAt, $nowTs) -gt 0)") }
        ...
        if ($debugMode) { [System.Console]::WriteLine("DEBUG: file=$($mf.Name) cmdPattern=$cmdPattern matchFail=$($Cmd -cnotmatch $cmdPattern)") }
        ...
        if ($debugMode) { [System.Console]::WriteLine("DEBUG: file=$($mf.Name) rawTextSnippet=$($rawText.Substring(0, [Math]::Min(100, $rawText.Length))) arrayCheckFail=$($rawText -cnotmatch '\"authorized_operations\"\s*:\s*\[')") }
        ...
        if ($debugMode) { [System.Console]::WriteLine("DEBUG: file=$($mf.Name) cmdType=$cmdType opsCount=$opsCount opVal=$opVal") }
        ...
        if ($debugMode) { [System.Console]::WriteLine("DEBUG: adding candidate file=$($mf.Name)") }
        $candidates.Add("${issuedAt}|$($mf.FullName)")
    }

    [System.Console]::WriteLine("PS1_CANDIDATES count=$($candidates.Count)")
    # No valid candidates found → deny
    if ($candidates.Count -eq 0) {
        if ($debugMode) { [System.Console]::WriteLine("DEBUG: no candidates found, returning false") }
        return $false
    }
