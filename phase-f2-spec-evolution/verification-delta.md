...
| **VP-HOOK-035** *(NEW v1.20, **P0**; ADV-F2-P18-005 OBS→RESOLVED / D-021 — §3.4 close scope + gating; **EXTENDED v1.21 — D-023 disposition gate [P19-001] + emit-time close-state [P19-003]**)* | BC-3.03.001 (disposition-guard `["close"]` marker emit + **D-023 disposition gate + emit-time CLOSE-STATE re-check** + STEP-4 hard-floor DENY-THE-WRITE + STEP-5 kill switch + close ticket_id charset) + BC-10.01.001 (§3.4 `close` authorized_operations / `close` ticket_action_type, **EC-013 3-condition AND (D-023)**, hard-floor + kill-switch gated, config-driven close-state allowlist, D-021/D-023); consumed by BC-3.01.001 (require-review close write-block + anti-fungibility) | none (max prior VP-HOOK = 036; EXTENDED in place v1.21, no new id; FRESH `grep -rn 'VP-HOOK-037' .factory/` confirmed absent repo-wide) | **FINALIZED** (**v1.21 D-023: a `["close"]` marker is NOT issued unless verdict.disposition∈{FP,BTP} — TP/Indeterminate + ticket_action_type=close → `CLOSE-DISPOSITION-DENY` as the FIRST close-branch check, regardless of autonomy_enabled/scored_priority; v1.21 P19-003: emit-time `CLOSE-STATE-DENY` re-check, missing config defaults to 'Done';** a `["close"]` marker authorizes ONLY `jr issue move <ticket_id> <allowlisted_close_state>`, state ∈ CLOSE_STATE_ALLOWLIST={Done,Closed,Resolved} config-driven/NOT verdict-influenceable; the close marker is issued ONLY for FP/BTP + non-hard-floor + autonomy_enabled=true; HIGH/CRIT scored_priority FP/BTP → STEP-4 DENY-THE-WRITE (no close marker → comment-review); autonomy_enabled=false → STEP-5 kill switch; non-allowlisted state → deny; close ticket_id charset (`TICKET-ID-CHARSET-DENY`, O7 close site); bidirectional anti-fungibility; paired mutants SM-61 [hard-floor bypass], SM-62 [kill-switch removed], SM-63 [state-allowlist removed], SM-64 [close ticket_id O7 removed], **SM-66 [disposition-gate removed], SM-67 [emit-time close-state re-check removed]**) |
| **VP-HOOK-036** *(NEW v1.20, **P0**; ADV-F2-P18-003 MEDIUM / D-022 — compound-action model; **EXTENDED v1.21 — rule-2 create+link [P19-004 HUMAN DECISION] + orphan-link reconciliation [P19-002]**)* | BC-10.01.001 (Stage-7 two-sequential-verdict-Write compound-action model, §3.4 rule 2 [**create+link, P19-004**] / rule 4, **orphan-link reconciliation P19-002**, D-022/P19-002/P19-004) + BC-3.03.001 (per-verdict single-element marker emit) + BC-3.01.001 (single-use marker consume) | none (max prior VP-HOOK = 036; EXTENDED in place v1.21, no new id; FRESH `grep -rn 'VP-HOOK-037' .factory/` confirmed absent repo-wide) | **FINALIZED** (compound §3.4 actions — **v1.21: rule 2 create+link (P19-004 HUMAN DECISION — parallel to rule 4, NOT comment+link)**, rule 4 create+link — are TWO sequential Stage-7 verdict Writes, each a single-element single-use marker, NOT one multi-marker; the link verdict carries ticket_id=NEW_KEY from the preceding create (loop obligation); **v1.21 orphan-link reconciliation (P19-002): a create-landed/link-never-landed ticket is re-linked (LINK verdict only, no re-create) on a subsequent loop run when §3.4 dedup detects the missing 'Relates' link; `jr issue link` idempotent;** the never-multi-element invariant preserved; paired mutants SM-65 [single marker authorizes both writes → dies on single-use consume], **SM-68 [orphan-link reconciliation removed → ticket stays unlinked on re-run]**) |
...
with the NEW `LINK-TARGET-CHARSET-DENY` deny code; no existing VP renumbered/re-scoped**; **v1.21 adds NO new VP — VP-HOOK-035 EXTENDED in place (D-023 close disposition gate [P19-001, +SM-66] + emit-time close-state validation [P19-003, +SM-67]) and VP-HOOK-036 EXTENDED in place (§3.4 rule-2 create+link [P19-004 HUMAN DECISION — replaces comment+link] + orphan-link reconciliation [P19-002, +SM-68]); VP count stays 41; no existing VP renumbered/re-scoped**).
...
before command_pattern construction (D-020/O7, SM-59/SM-60); VP-HOOK-035 close `["close"]` marker authorization gated by **D-023 disposition gate (v1.21 — TP/Indeterminate close → CLOSE-DISPOSITION-DENY, FIRST close-branch check regardless of autonomy/scored_priority)** + `hard_floor_applies()` (HIGH/CRIT
FP/BTP → STEP-4 DENY-THE-WRITE → comment-review) + kill switch (autonomy=false → STEP-5 no marker) + config-driven close-state allowlist + **emit-time close-state re-check (v1.21 P19-003)** + close ticket_id
O7 (D-021/D-023/P18-005/P19-001/P19-003, SM-61/SM-62/SM-63/SM-64/**SM-66/SM-67**); VP-HOOK-036 the D-022 compound-action model — compound §3.4 actions (**rule 2 create+link [v1.21 P19-004 HUMAN DECISION — was comment+link]**, rule 4 create+link)
are TWO sequential single-use verdict Writes, NOT one multi-marker, + **orphan-link reconciliation (v1.21 P19-002 — create-landed/link-orphaned → re-link only on re-run)** (P18-003/P19-002/P19-004, SM-65/**SM-68**)**; **VP-HOOK-032 is the NEW v1.15 P0 O7 command_pattern interpolation-safety property —
...
re-target/renumber; SM-55 remains reserved-skipped.** All 61
...
| **SM-66** *(NEW v1.21; occupancy-verified — pre-appeared ONLY in architecture-delta.md + adversarial-spec-delta-review-pass19.md as the architect's RESERVED placeholder, NOT a live verif-delta mutant; FV claims it here per that §8.33 reservation; next-free real SM after SM-65)* | disposition-guard emitter close branch — D-023 disposition gate (D-023 / P19-001) | **close-disposition-gate-removed** (ADV-F2-P19-001 CRITICAL) — remove the D-023 disposition gate (the FIRST check in the close branch, `IF verdict.disposition NOT IN {"FP","BTP"}: emit deny(CLOSE-DISPOSITION-DENY); RETURN`, placed before the ticket_id null check) so a verdict whose `ticket_action_type=close` but whose `disposition ∈ {TP,Indeterminate}` is NOT cross-validated → a **TP verdict flows STEP 4 (no floor — TP is not a hard-floor category) → STEP 5 (kill switch passes) → close branch → issues a `["close"]` marker** → a confirmed-malicious ticket is auto-closed (the O3 regression since D-021). | **VP-HOOK-035 TP-close-deny vector** (disposition=TP + `scored_priority=LOW` + `ticket_action_type=close` + `autonomy_enabled=true` → `CLOSE-DISPOSITION-DENY`, verdict Write denied, NO `["close"]` marker) — under the mutant the gate is gone → the TP close issues a marker and the mutant dies; the disposition=BTP positive control confirms the gate does NOT over-fire for FP/BTP. Fires regardless of autonomy_enabled/scored_priority. Distinct from SM-61 (the `hard_floor_applies()` close-scope bypass) — SM-66 is the disposition↔action coherence gate, not a hard-floor gate |
...
| **SM-68** *(NEW v1.21; occupancy-verified — `grep -rn 'SM-68' .factory/` returned no match before allocation; next-free after SM-67)* | monitoring-loop §3.4 compound-action orphan-link reconciliation (P19-002 / D-022 partial-failure) | **orphan-link-reconciliation-removed** (ADV-F2-P19-002 MINOR) — remove the D-022 idempotent orphan-link reconciliation so that when a compound create+link (rule 2 or rule 4) lands verdict-1 (create) but NOT verdict-2 (link) — process interruption, loop crash, or a subsequently denied Write — the subsequent loop run's §3.4 dedup does NOT detect / does NOT re-issue the missing link: the NEW ticket stays PERMANENTLY unlinked to its companion (audit + operational-integrity gap). | **VP-HOOK-036 orphan-link reconciliation vector** (create landed / link marker never landed → on re-run, §3.4 dedup detects the open ticket lacking the expected 'Relates' link → loop re-issues the **LINK verdict ONLY** → `jr issue link NEW_KEY SEC-99` ALLOW, NO second `["create"]` marker) — under the mutant the loop never re-links and the ticket stays unlinked, so the reconciliation assertion fails and the mutant dies; the idempotent-re-link control (`jr issue link` no-op when the link already exists) confirms re-runs are safe. Non-security (the finding exists; only the link is missing) |

**New mutation vector count: 61 (SM-9 … SM-68, SM-55 reserved-skipped; SM-32 = SM-32a + SM-32b + SM-32-ext as of v1.9; SM-32a re-targeted + SM-32-ext kill vector re-worded at v1.10; +SM-38/SM-39/SM-40 at v1.10; +SM-41/SM-42 at v1.11; +SM-43 at v1.12; +SM-44/SM-45 at v1.13; +SM-46/SM-47 at v1.14; +SM-48/SM-49/SM-50/SM-51 at v1.15; +SM-52/SM-53 at v1.16; +SM-54 at v1.17; SM-55 SKIPPED at v1.18 (reserved-skipped); +SM-56 at v1.19; **+SM-57/SM-58/SM-59/SM-60/SM-61/SM-62/SM-63/SM-64/SM-65 at v1.20** [SM-57/58 link authorization (VP-HOOK-033), SM-59/60 link KEY1/KEY2 O7 charset (VP-HOOK-034), SM-61/62/63/64 close hard-floor/kill-switch/state-allowlist/ticket_id-O7 (VP-HOOK-035), SM-65 compound single-use (VP-HOOK-036)]; **+SM-66/SM-67/SM-68 at v1.21** [SM-66 close-disposition-gate (D-023/VP-HOOK-035), SM-67 emit-time-close-state-recheck (P19-003/VP-HOOK-035), SM-68 orphan-link-reconciliation (P19-002/VP-HOOK-036)]).** SM-9..SM-16,
...
1. **P19-001 / D-023 — close DISPOSITION gate (MY doc; VP-HOOK-035 EXTENDED).** VP-HOOK-035 now asserts a `["close"]` marker is
   ...
   `CLOSE-DISPOSITION-DENY` as the FIRST check in the emitter close branch (before the ticket_id null check), regardless of
   ...
   verdict flowed STEP 4 no-floor → STEP 5 kill-switch-passes → unguarded close). EC-013 restored to the full 3-condition AND:
   disposition∈{FP,BTP} AND hard_floor_applies()=false AND autonomy_enabled=true. Paired mutant SM-66 (disposition gate removed —
   claims the architect's §8.33 reservation) killed by the TP-close-deny vector.
2. **P19-001 / D-023 — BC-side emitter close-branch disposition gate (PO-owned, in parallel; §8.33.1).** The PO adds the
   `CLOSE-DISPOSITION-DENY` gate to the BC-3.03.001 emitter close branch and restores EC-013's 3-condition AND in BC-10.01.001.
   FV does NOT edit the BCs; VP-HOOK-035's BC anchors cite D-023/P19-001.
...
none alters an invariant/EC/PC beyond restoring EC-013's 3-condition AND**). All delta BCs are otherwise
...
*F2 Verification Delta v1.21 complete. **41 VPs** (0 collisions, 0 renumbering — VP count UNCHANGED at 41; VP-HOOK-035 +
...
ticket_action_type=close → `CLOSE-DISPOSITION-DENY` as the FIRST close-branch check, regardless of autonomy_enabled/scored_priority
(EC-013 restored to the 3-condition AND). Paired mutant SM-66 (disposition gate removed → a TP close issues a marker → dies on the
...
**BC-3.03.001 v1.28** (emitter close branch — disposition gate + emit-time close-state), **BC-10.01.001 v1.22** (§3.4 EC-013 3-condition AND +
...
COMPOUND-ACTION (VP-HOOK-036):** compound §3.4 actions (rule 2 comment+link; rule 4 create+link) are TWO sequential Stage-7
