...
4. **[UPDATED v1.12; corrected v1.19 P11-004] EMITTER role — this hook is the ONLY marker issuance path (D-DEC-001).** After ICD-203 validation passes (postcondition #1 — verdict-class path), the hook enters the emitter decision tree. The emitter is entered ONLY from the verdict-class (PC#1 JSON path). The investigation-markdown path (PC#2) does NOT enter this emitter — it uses a SEPARATE minimal comment-scoped marker path (P11-004 — see 'Separate Human-Comment Marker Path' section within this invariant for the pseudocode). The emitter reads `verdict.ticket_action_type` (field 15) to select the marker scope branch.
   ...
   **Emitter decision tree (D-DEC-008 v1.6 — full pseudocode):**
   ...
   IF action == "link" AND hard_floor_applies(verdict, recomputed_severity):
     ...
     # P23-004: subroutine replaces GOTO STEP6_LINK — no GOTO into ELIF ladder (ill-formed).
   ...
   link_target = null   # schema v2.2: non-null only for ["link"] scope; initialized here so WRITE_MARKER always has this variable defined
   IF action == "comment":
     ticket_id = verdict.ticket_id
   ...
   ELIF action == "link":
     ...
     # null-binding guards applied, then EMIT_LINK_MARKER(is_hard_floor_link=true) called;
     # this ELIF is NOT reached from STEP 3b.
     # P23-004: GOTO STEP6_LINK (ill-formed GOTO into ELIF ladder) replaced with named
     # subroutine EMIT_LINK_MARKER callable from both STEP 3b and STEP 6.
     # O7 charset validation, org-binding (D-028/P23-005), and pattern construction
     # reside in EMIT_LINK_MARKER — single implementation for both entry paths.
   ...
   FUNCTION EMIT_LINK_MARKER(verdict, recomputed_severity, is_hard_floor_link):
     ...
     IF resolved_project_key is null:
       ...
       RETURN
     # KEY1 must match <resolved_project_key>-<digits> (check uses pre-escape ticket_id)
     IF NOT regex_match("^" + resolved_project_key + "-[0-9]+$", verdict.ticket_id):
       WRITE audit entry:
       ...
       RETURN
     # KEY2 must match <resolved_project_key>-<digits> (check uses pre-escape ticket_id_b)
     IF NOT regex_match("^" + resolved_project_key + "-[0-9]+$", verdict.link_target_ticket_id):
       WRITE audit entry:
   ...
   # EMIT_LINK_MARKER falls through to WRITE_MARKER below.
   ...
   # D-028: is_review_path extended to hard-floor link path (see WRITE_MARKER below).
   WRITE_MARKER:
   expires_at = now() + 120s
   # D-028 (P23-001): is_review_path extended to cover hard-floor link path.
   # is_link_hard_floor is set by EMIT_LINK_MARKER; default false for non-link paths.
   ...
   marker = {
     ...
     link_target_ticket_id: link_target,  # schema v2.2: non-null only for ["link"] scope; null for all others
     disposition: {
       verdict: verdict.disposition,
   ...
   IF NOT write_ok:
     IF is_review_path:
       ...
       WRITE audit entry:
         ...
         " verdict=" + verdict.disposition + "/" + recomputed_severity + " (P10-003/D-028)"
       emit deny(
   ...
   **P12-001/P18-001/P19-003/O7 charset-validation at all interpolation sites:** Before any `ticket_id`, `jira_project_key`, or `link_target_ticket_id` value is interpolated into a `command_pattern`, it is validated against a fixed charset (`ticket_id` / `link_target_ticket_id` → `^[A-Z][A-Z0-9]+-[0-9]+$`; `jira_project_key` → `^[A-Z][A-Z0-9]+$`) with hard deny on mismatch (TICKET-ID-CHARSET-DENY / LINK-TARGET-CHARSET-DENY / PROJECT-KEY-CHARSET-DENY). `regex_escape()` applied as defense-in-depth after the charset check. **9 active O7 sites total (P18-005 + P19-003):** site 1 (STEP 3 comment-review ticket_id), site 2 (STEP 3 create-review jira_project_key), site 3 (STEP 6 link ticket_id KEY1), site 4 (STEP 6 close ticket_id → `ticket_id_safe`), site 5 (STEP 6 comment ticket_id), site 6 (STEP 6 create jira_project_key), site 7 (STEP 6 assign ticket_id), site 8 (STEP 6 link link_target_ticket_id KEY2), site 9 (STEP 6 close `jira_close_state` → `close_state_safe` — P19-003 belt-and-suspenders: CONFIG-driven value, emit-time CLOSE_STATE_ALLOWLIST re-checked + `regex_escape(close_state)` applied; setup-time validation + emit-time re-check + emit-time escape = three-layer protection). This applies to all 7 rows that construct a `command_pattern` (comment, comment-review, assign, create, create-review, link, close) — see STEP 3 and STEP 6 pseudocode above.
   ...
   **Separate Human-Comment Marker Path (P11-004 / redesigned P12-002 / P22-001):**

   This sub-path applies ONLY to `investigation-*.md` files dispatched via PC#2. It does NOT enter the verdict emitter above. `validate_enums()` and STEP 1a (NORMALIZE_SEVERITY / SEVERITY-MISMATCH) are NOT called on this path. Per P22-001, disposition routing runs FIRST (before any kill-switch check): FP → allow-without-marker; non-FP/PARSE_FAIL → MARKDOWN_REVIEW_PATH (EXEMPT from kill switch per D-DEC-012 Option A regardless of `autonomy_enabled`). The gating sequence is: (1) 12-field completeness check (heading-anchored grep), (2) GATE 1/GATE 2 markdown-evaluable hard floors (Indeterminate, forbidden technique, degraded/silent sensor), (3) disposition routing.

   ```
   # ── SEPARATE HUMAN-COMMENT MARKER PATH (P11-004 / redesigned P12-002 / P13-001 / P22-001) ──
   ...
   #
   # P22-001 FIX — DISPOSITION ROUTING RUNS FIRST (mirroring verdict path STEP 3 before STEP 5):
   ...
   # This was the exact D-025/P20-001 ordering pathology reproduced on the markdown path.
   # Fix: autonomy_enabled is no longer the first check. Disposition routing runs FIRST.
   # FP → allow-without-marker regardless of autonomy_enabled (kill switch irrelevant).
   ...
   #
   # ── GATE 1 / GATE 2: markdown-evaluable hard floors (P11-004, renumbered from GATES 2/3) ────
   ...
   **Trust basis (P11-004 / P12-002 / P13-001 / P22-001):** Gate 1/2 (P11-004, renumbered): markdown-evaluable floors (Indeterminate disposition, forbidden techniques T1003/T1068/T1021/T1041, degraded/silent sensor) — run BEFORE disposition routing per P22-001. Disposition routing (P22-001 — runs FIRST, before any kill-switch check): `parsed_disposition == "FP"` → allow-without-marker (hook cannot evaluate scored_priority/asset_type from 12-field markdown; no known-FP store cross-check; kill switch irrelevant — FP always allow-without-marker); `parsed_disposition != "FP"` or PARSE_FAIL → MARKDOWN_REVIEW_PATH (create-review/comment-review; EXEMPT from kill switch per D-DEC-012 Option A regardless of autonomy_enabled — P22-001). No disposition yields an autonomous comment marker (P13-001 MARKDOWN_COMMENT_PATH ELIMINATED). `autonomy_enabled` is no longer an independent gate; it is irrelevant for FP (allow-without-marker regardless) and irrelevant for non-FP/PARSE_FAIL (MARKDOWN_REVIEW_PATH regardless). `parse_autonomy_enabled_from_markdown` (P13-003): reads ONLY a dedicated structured field; embedded tokens in code fences/evidence blocks return false — this guard still applies when constructing the adversarial-autonomy-masquerade test case, but does not participate in the primary routing sequence. Verdict-only fields (`scored_priority`, `asset_type`, `verdict.severity`) are NOT present in a standard ICD-203 investigation markdown and are NOT checked on this path. `ticket_id` parsed from free-text markdown is charset-validated (P12-001/O7) before interpolation.
   ...
   **Verification property (VP-HOOK-031 — FINALIZED P0, per verification-delta.md v1.14; UPDATED v1.15 P12-002 SM-50/SM-51; SCOPE UPDATE COMPLETE v1.21 P13-001 — SM-51 (route-to-review, reconciled) / SM-52 (FP-comment-marker revert); P22-001 SCOPE UPDATE — disposition-routing-first [ID-sync per FV]):** Separate human-comment marker path correctness (P11-004 / P12-002 / P13-001 / P22-001): the 12-field ICD-203 investigation-markdown path (PC#2 dispatch) does NOT enter the verdict emitter; `validate_enums()` and STEP 1a are NOT called on this path. Gate 1/2 (P11-004, renumbered): markdown-evaluable floors → MARKDOWN-HARD-FLOOR deny. **P22-001 disposition-routing-first:** disposition routing runs BEFORE any kill-switch check — FP → allow-without-marker (kill switch irrelevant; MARKDOWN_COMMENT_PATH ELIMINATED per P13-001); non-FP/PARSE_FAIL → MARKDOWN_REVIEW_PATH (create-review/comment-review — EXEMPT from kill switch per D-DEC-012 Option A regardless of autonomy_enabled). SM-52 kill target: restoring MARKDOWN_COMMENT_PATH causes FP+autonomy_enabled=true to emit ["comment"] marker — incorrect. P22-001 SM target (TBD): reverting P22-001 — making autonomy_enabled absent cause allow-without-marker for TP — causes non-FP investigation markdown with autonomy_enabled absent to exit without review marker (incorrect — should route to MARKDOWN_REVIEW_PATH). Adversarial parse vectors (P13-003): "Disposition: not a false positive" → PARSE_FAIL → review (NOT allow-without-marker); `autonomy_enabled: true` embedded in code fence → parse_autonomy_enabled_from_markdown returns false (embedded token does not match dedicated structured field) — relevant for P13-003 adversarial masquerade test but NOT a routing gate in the P22-001 sequence. Consumed by BC-5.01.001 (investigate-event Invariant #7) and BC-4.02.001 (update-jira PC#4). Paired mutant SM-47 (markdown-routed-into-verdict-emitter): routes investigation-markdown into verdict emitter — kills compliant-save-allowed and no-validate_enums vectors. **NOTE (P13-001): SM-P12-D ("revert P12-002 — remove disposition routing rule; issue comment marker for all dispositions") is SUPERSEDED by SM-51 (route-to-review, reconciled) — the correct behavior for TP/BTP/PARSE_FAIL routes to review. SM-52 (FP-comment-marker revert, P13-001) covers the FP path: "revert P13-001 — restore MARKDOWN_COMMENT_PATH: FP+autonomy_enabled=true issues autonomous comment marker" → kill target.** [ID-sync per FV]
...
| `investigation-ALERT-001.md` → all 12 mandatory headings, Disposition=Indeterminate (autonomy_enabled absent) | **P22-001 routing (disposition-routing-first):** GATE 1 fires (Indeterminate → MARKDOWN-HARD-FLOOR); `permissionDecision: deny`; deny reason "MARKDOWN-HARD-FLOOR: Indeterminate disposition"; audit entry written; verdict emitter NOT entered. `autonomy_enabled` absent is no longer a routing gate — hard floors now run BEFORE disposition routing per P22-001. **Previous (v1.26 / pre-P22-001):** "Gate 1 fires — autonomy_enabled absent → allow-without-marker (kill-switch parity)" — that was the P22-001 defect: Indeterminate should be MARKDOWN-HARD-FLOOR denied regardless of autonomy_enabled; old GATE 1 masked the hard floor by firing first. | **edge-case (P22-001 reorder — Indeterminate + autonomy_enabled absent → GATE 1 hard floor fires → deny; was: allow-without-marker pre-P22-001)** |
| `investigation-ALERT-001.md` → all 12 mandatory headings, **autonomy_enabled: true**, Disposition=Indeterminate, Sensor Health Status=healthy | Separate Human-Comment Marker Path (P11-004 / P22-001): GATE 1 fires (Indeterminate → MARKDOWN-HARD-FLOOR); `permissionDecision: deny`; deny reason "MARKDOWN-HARD-FLOOR: Indeterminate disposition"; audit entry written; verdict emitter NOT entered. (`autonomy_enabled: true` does not affect this outcome — GATE 1/2 hard floors run before disposition routing in P22-001 sequence.) | **edge-case (P11-004 GATE 1 — MARKDOWN-HARD-FLOOR, Indeterminate; autonomy_enabled value irrelevant)** |
...
| `verdict-ALERT-001.json` → JSON with all 18 keys, disposition=Indeterminate, **sensor_health_status=silent** (BLIND-SPOT), scored_priority=HIGH, ticket_action_type=link, ticket_id=PRISMDEMO-42, link_target_ticket_id=PRISMDEMO-OLD, **autonomy_enabled=false** (or any value) | **P22-003/D-027/D-028 STEP 3b canonical vector (BLIND-SPOT hard-floor link):** `permissionDecision: allow`; STEP 1/1a pass (Indeterminate + silent → consistent); STEP 2 pass; STEP 3: Indeterminate + silent NOT a review-labeled token → falls through STEP 3; STEP 3b fires: action="link" AND hard_floor_applies()=TRUE (Indeterminate + silent sensor) → D-028 null-binding guards: ticket_id_step3b=PRISMDEMO-42 (non-null, passes), ticket_id_b_step3b=PRISMDEMO-OLD (non-null, passes); EMIT_LINK_MARKER(is_hard_floor_link=true) called; STEP 4 and STEP 5 bypassed (autonomy_enabled=false irrelevant — STEP 3b fires before STEP 5); O7 charset + org-binding checks run in EMIT_LINK_MARKER (P23-004); is_link_hard_floor=true set; link-scoped marker written (authorized_operations=["link"], command_pattern `^jr (--output json )?issue link PRISMDEMO-42 PRISMDEMO-OLD( \|$)`, link_target_ticket_id="PRISMDEMO-OLD", ticket_id="PRISMDEMO-42"); WRITE_MARKER: is_review_path=true (D-028) — marker-write success → allow. **Pre-D-027 (v1.30):** STEP 4 fires → HARD-FLOOR-UNDER-LABEL deny (P22-003 CRITICAL gap). **Pre-D-028 (v1.31):** GOTO STEP6_LINK used (P23-004 ill-formed); no null-binding guards; no org-binding; is_review_path=false for link. | **edge-case (P22-003/D-027/D-028 STEP 3b — BLIND-SPOT hard-floor link: STEP 3b fires; null-binding guards pass; EMIT_LINK_MARKER(is_hard_floor_link=true); STEP 4+5 bypassed; ["link"] marker issued regardless of autonomy_enabled)** |
