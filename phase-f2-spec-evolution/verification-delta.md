...
| **VP-HOOK-035** *(NEW v1.20, **P0**; ADV-F2-P18-005 OBS→RESOLVED / D-021 — §3.4 close scope + gating)* | BC-3.03.001 (disposition-guard `["close"]` marker emit + STEP-4 hard-floor DENY-THE-WRITE + STEP-5 kill switch + close ticket_id charset) + BC-10.01.001 (§3.4 `close` authorized_operations / `close` ticket_action_type, hard-floor + kill-switch gated, config-driven close-state allowlist, D-021); consumed by BC-3.01.001 (require-review close write-block + anti-fungibility) | none (max prior VP-HOOK = 034 this pass; FRESH `grep -rn 'VP-HOOK-035' .factory/` confirmed absent repo-wide) | **FINALIZED** (a `["close"]` marker authorizes ONLY `jr issue move <ticket_id> <allowlisted_close_state>`, state ∈ CLOSE_STATE_ALLOWLIST={Done,Closed,Resolved} config-driven/NOT verdict-influenceable; the close marker is issued ONLY for FP/BTP + non-hard-floor + autonomy_enabled=true; HIGH/CRIT scored_priority FP/BTP → STEP-4 DENY-THE-WRITE (no close marker → comment-review); autonomy_enabled=false → STEP-5 kill switch; non-allowlisted state → deny; close ticket_id charset (`TICKET-ID-CHARSET-DENY`, O7 close site); bidirectional anti-fungibility; paired mutants SM-61 [hard-floor bypass], SM-62 [kill-switch removed], SM-63 [state-allowlist removed], SM-64 [close ticket_id O7 removed]) |
| **VP-HOOK-036** *(NEW v1.20, **P0**; ADV-F2-P18-003 MEDIUM / D-022 — compound-action model)* | BC-10.01.001 (Stage-7 two-sequential-verdict-Write compound-action model, §3.4 rule 2 / rule 4, D-022) + BC-3.03.001 (per-verdict single-element marker emit) + BC-3.01.001 (single-use marker consume) | none (max prior VP-HOOK = 035 this pass; FRESH `grep -rn 'VP-HOOK-036' .factory/` confirmed absent repo-wide) | **FINALIZED** (compound §3.4 actions — rule 2 comment+link, rule 4 create+link — are TWO sequential Stage-7 verdict Writes, each a single-element single-use marker, NOT one multi-marker; rule 4's link verdict carries ticket_id=NEW_KEY from the preceding create (loop obligation); the never-multi-element invariant preserved; paired mutant SM-65 [single marker authorizes both writes → dies on single-use consume]) |
...
with the NEW `LINK-TARGET-CHARSET-DENY` deny code; no existing VP renumbered/re-scoped**).
...
before command_pattern construction (D-020/O7, SM-59/SM-60); VP-HOOK-035 close `["close"]` marker authorization gated by `hard_floor_applies()` (HIGH/CRIT
FP/BTP → STEP-4 DENY-THE-WRITE → comment-review) + kill switch (autonomy=false → STEP-5 no marker) + config-driven close-state allowlist + close ticket_id
O7 (D-021/P18-005, SM-61/SM-62/SM-63/SM-64); VP-HOOK-036 the D-022 compound-action model — compound §3.4 actions (rule 2 comment+link, rule 4 create+link)
are TWO sequential single-use verdict Writes, NOT one multi-marker (P18-003, SM-65)**; **VP-HOOK-032 is the NEW v1.15 P0 O7 command_pattern interpolation-safety property —
...
SM-32-ext). No re-target/renumber; SM-55 remains reserved-skipped.** All 58
...
**New mutation vector count: 58 (SM-9 … SM-65, SM-55 reserved-skipped; SM-32 = SM-32a + SM-32b + SM-32-ext as of v1.9; SM-32a re-targeted + SM-32-ext kill vector re-worded at v1.10; +SM-38/SM-39/SM-40 at v1.10; +SM-41/SM-42 at v1.11; +SM-43 at v1.12; +SM-44/SM-45 at v1.13; +SM-46/SM-47 at v1.14; +SM-48/SM-49/SM-50/SM-51 at v1.15; +SM-52/SM-53 at v1.16; +SM-54 at v1.17; SM-55 SKIPPED at v1.18 (reserved-skipped); +SM-56 at v1.19; **+SM-57/SM-58/SM-59/SM-60/SM-61/SM-62/SM-63/SM-64/SM-65 at v1.20** [SM-57/58 link authorization (VP-HOOK-033), SM-59/60 link KEY1/KEY2 O7 charset (VP-HOOK-034), SM-61/62/63/64 close hard-floor/kill-switch/state-allowlist/ticket_id-O7 (VP-HOOK-035), SM-65 compound single-use (VP-HOOK-036)]).** SM-9..SM-16,
...
metadata (non-ICD-203), and the config-driven close-state allowlist is NOT verdict-influenceable**). All delta BCs are otherwise
...
close-state allowlist, P18-005/D-021), VP-HOOK-036 (compound-action two-sequential-verdict-Write model, P18-003/D-022)**)
and the SM-N catalog (SM-9..**SM-65**, **58 mutants** with SM-32 = SM-32a + SM-32b + SM-32-ext, **SM-55 reserved-skipped**; +SM-38/SM-39/SM-40 at v1.10; +SM-41/SM-42 at v1.11; +SM-43 at v1.12; +SM-44/SM-45 at v1.13; +SM-46/SM-47 at v1.14; +SM-48/SM-49/SM-50/SM-51 at v1.15; **+SM-52/SM-53 at v1.16; +SM-54 at v1.17; SM-55 SKIPPED at v1.18; +SM-56 at v1.19; +SM-57..SM-65 at v1.20**).
...
*F2 Verification Delta v1.20 complete. **41 VPs** (0 collisions, 0 renumbering — VP count 37 → 41, +VP-HOOK-033/034/035/036,
